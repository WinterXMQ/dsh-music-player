...
3. **每日定时**：对 agent 说「每天早上 9 点播报新闻」即可；也可以在「新闻播报」页签的「⏰ 每日定时」里可视化配置**多班次**（如 08:00 / 12:30 / 18:00），每个班次可独立指定收集范围，并勾选「收集后立即播放」（不勾选 = **静默收集**，只更新简报不出声）。配置保存后点「**同步到定时任务**」——插件会把同步指令自动注入当前会话，agent 自动创建/更新定时任务，**无需手动打字**。建议在专用的「新闻简报」会话里操作——定时任务与每轮收集都归属创建它的会话，专用会话可不污染主工作对话。
...
**新闻定时任务设置了却不触发？**
定时任务归属于**创建它的会话**（session-local）：请确认你是在常驻的那个会话里同步的；主机进程（dsh-desktop / dsh 服务）必须保持运行，错过触发后重启会补跑最新一次。同步状态可在「新闻播报」页签的「⏰ 每日定时」里查看（未同步/已同步）。
