...
3. **每日定时**：对 agent 说「每天早上 9 点播报新闻」即可；也可以在「新闻播报」页签的「⏰ 每日定时」里可视化配置**多班次**（如 08:00 / 12:30 / 18:00），每个班次可独立指定收集范围，并勾选「收集后立即播放」（不勾选 = **静默收集**，只更新简报不出声）。配置保存后对 agent 说「同步新闻定时」即可生效。**建议同步在专用的「新闻简报」会话里做**——定时任务与每轮收集都归属创建它的会话，专用会话可不污染主工作对话。
4. **手动补收**：定时班次行有「▶ 立即执行」（一键复制指令，发给 agent 即按该班次配置立刻跑一轮）；同一班次 10 分钟内重复收集会被自动跳过。
