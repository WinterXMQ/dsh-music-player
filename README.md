...
**新闻定时任务设置了却不触发？**
定时器由插件在 DSH 主机进程内自维护（读面板已保存的班次偏好，每 30s 检查一次，到点触发）——**请确认 dsh-desktop / dsh 服务主机进程保持运行**；浏览器没开不影响收集（只影响出声）。每次触发会新建一个执行会话去收集并绑定结果。可在「新闻播报」页签的「⏰ 每日定时」里查看/修改班次；保存即生效。
