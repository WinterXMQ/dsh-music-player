...
光听歌还不够，它还能**听书**：把本地 `.txt`/`.epub` 小说丢给 AI 朗读，想听哪章点哪章、声音随便挑。现在还能**听新闻**：让 agent 用联网搜索收集当天头条（热点/国内/国际/科技/财经/体育，或 AI 等任意自定义主题），整理筛选成口播稿用 AI 声音播报——支持每日多班次定时、静默收集、文字版阅读。最绝的是它注册了 `music_play` 等模型工具——你连鼠标都不用动，在对话框里跟 agent 说句「播放周杰伦的歌」，音乐分分钟响起来，摸鱼摸出新境界。
...
1. **对话即时播报**（收集完立即播放）：对 agent 说「**播报今天的新闻**」。它会按类别搜索当天头条（默认热点/国内/国际/科技/财经/体育，可任意指定，如「播一下 AI 相关的新闻」）、跨源去重、每条写口播摘要并标注来源，然后自动开播；说「收集一下今天的新闻，先别播」则只生成不出声。
...
3. **每日定时**：对 agent 说「每天早上 9 点播报新闻」即可；也可以在「新闻播报」页签的「⏰ 每日定时」里可视化配置**多班次**（如 08:00 / 12:30 / 18:00），每个班次可独立指定收集范围，并勾选「收集后立即播放」（不勾选 = **静默收集**，只更新简报不出声）。配置保存后点「**同步到定时任务**」——插件会自动**创建/复用专用「新闻简报」会话**（无需你手动开会话）并把同步指令注入其中，agent 自动创建/更新定时任务，**无需手动打字**。你还可以在「⏰ 每日定时」里给专用会话选一个**新闻会话模型**（不选则跟随当前活跃会话）。定时任务与每轮收集都归属这个专用会话，不污染主工作对话。
4. **手动补收**：定时班次行有「▶ 立即执行」——一键自动触发 agent 按该班次配置立刻跑一轮（不等时刻）；同一班次 10 分钟内重复收集会被自动跳过。
...
**新闻定时任务设置了却不触发？**
定时任务归属于**创建它的会话**（session-local）。现在「同步到定时任务」会自动创建/复用专用「新闻简报」会话并承载定时任务——请确认你是在常驻主机进程里同步过（dsh-desktop / dsh 服务必须保持运行，错过触发后重启会补跑最新一次）。同步状态与专用会话状态可在「新闻播报」页签的「⏰ 每日定时」里查看（未同步/已同步）。

**新闻期次为什么少了 / 旧的去哪了？**
每个班次（含手动收集）独立保留最近 7 期，超出自动裁剪；也可以在面板里手动删除。「未听」的期次会带「待播」徽标，方便快速找到漏掉的。
