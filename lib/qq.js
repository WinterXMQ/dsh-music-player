...
// =====================================================================
// 歌曲搜索：返回歌曲数组（兼容既有调用方：results[0].songmid / results.length）。
export async function search(keyword, cookie = '') {
  const url = `https://c.y.qq.com/soso/fcgi-bin/client_search_cp?${new URLSearchParams({ w: keyword, format: 'json', p: '1', n: '20', t: '0', cr: '1' }).toString()}`
...
export async function detectVip(cookie) {
  ...
  const body = urlGetVkeyBody(cookie, [songMID], ['M500'])
  const j = await postMusicu(body, cookie)
  const info = (j.req_1?.data?.midurlinfo || [])[0]
  return j.req_1?.code === 0 && !!info?.purl
}
...
export async function searchPlaylist(keyword, cookie = '') {
  const p = new URLSearchParams({ query: keyword, page_no: '0', num_per_page: '20', format: 'json', remoteplace: 'txt.yqq.playlist', flag_qc: '0' })
  const apiURL = `http://c.y.qq.com/soso/fcgi-bin/client_music_search_songlist?${p.toString()}`
...
export async function getMyPlaylists(cookie = '') {
  ...
  return list.filter((x) => x.tid && x.dirName).map((x) => ({
    id: String(x.tid), name: dec(x.dirName), cover: httpsCover(x.picUrl),
...
// 把歌曲加入/移出「我喜欢」歌单（dirId 固定 201）。song 需含 songid（数值）、songtype。
// 参考 qq-music-api 的 music.musicasset.PlaylistDetailWrite / AddSonglist(DelSonglist)。
async function qqFavWrite(song, cookie, method) {
  if (!cookie || !song) throw new Error('未登录，无法收藏')
  ...
  const j = await postMusicu(body, cookie)
  const res = j.req_0 || j['music.musicasset.PlaylistDetailWrite']
  const rawSnippet = (x) => { try { const s = JSON.stringify(x); return s ? s.slice(0, 300) : String(x) } catch { return String(x) } }
  const retCode = res?.data?.retCode
  if (retCode !== undefined) {
    if (Number(retCode) === 0) return true
    throw new Error('收藏接口失败（retCode=' + retCode + '，raw=' + rawSnippet(j) + '）')
  }
  const code = res ? Number(res.code) : (j.code !== undefined ? Number(j.code) : NaN)
  if (code === 0) return true
  throw new Error('收藏接口失败（code=' + code + '，raw=' + rawSnippet(j) + '）')
}
...
export async function getTopListSongs(topId, cookie = '') {
  const p = new URLSearchParams()
  p.set('topid', String(topId)); p.set('page', 'detail'); p.set('type', 'top'); p.set('tpl', '3')
  p.set('song_num', '30'); p.set('format', 'json')
  const apiURL = `https://c.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg?${p.toString()}`
  const res = await fetchWithTimeout(apiURL, { headers: { 'User-Agent': UA, 'Referer': 'https://y.qq.com/', ...(cookie ? { Cookie: cookie } : {}) } })
  let text = await res.text()
  if (text.trimStart().startsWith('MusicJsonCallback(')) { const i = text.indexOf('('); const e = text.lastIndexOf(')'); text = text.slice(i + 1, e) }
  const j = JSON.parse(text)
  const list = (j.songlist || []).map((x) => {
    const s = (x && x.data) || x || {}
    return {
      id: s.songmid, songmid: s.songmid, title: dec(s.songname || s.name || ''),
      songid: s.songid, songtype: s.songtype || 0,
  ...
  return {
    id: String(topId),
    name: dec((j.topinfo && (j.topinfo.ListName || j.topinfo.title)) || j.topTitle || ''),
    intro: dec((j.topinfo && (j.topinfo.info || j.topinfo.intro)) || ''),
    updateTime: j.date || '', total: j.total_song_num || list.length,
    songs: list,
  }
}
