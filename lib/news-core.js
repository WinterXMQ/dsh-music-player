...
export const LIMITS = {
  ...
  itemsPerCategory: 8,
  totalItems: 20,
  summaryChars: 1000, // 单条摘要上限：放足到不截断 agent 生成的真实新闻内容（避免出现省略号截断）
  titleChars: 60,
...
/** 单块（字幕单元）字符上限：一条完整新闻（标题+摘要）默认仍装进一个块，保证字幕面板一帧
 * 就是一条完整新闻；仅极长条目（> maxChars）才按句边界二次切分（内容完整、不截断）。 */
export const NEWS_ITEM_CHUNK_MAX = 600
