...
export const LIMITS = {
  ...
  itemsPerCategory: 8,
  totalItems: 20,
  summaryChars: 280, // 单条摘要上限：长一些的口播内容，避免每条新闻过短
  titleChars: 60,
...
export function renderScript({ title, date, categories, opening, closing }) {
  ...
  if (opening) {
    ...
    push('opening', `您好，这里是${title}，${formatDateCn(date)}。`)
  }
  // 类别与条目
...
/** 单块（字幕单元）字符上限：一条较长的完整新闻（标题+摘要 ≈ ≤380 字）默认仍装进一个块，
 * 保证字幕面板一帧就是一条完整新闻，同时容纳更充实的内容。 */
export const NEWS_ITEM_CHUNK_MAX = 400

/**
 * 按「一条新闻一个块」切分：opening / 每个 categoryLead / 每条 item / closing 各为一个块，
 * 这样字幕面板每次显示的都是完整的一条新闻（或一段引导语）。超长单元（罕见，超过
 * maxChars）才在句边界内二次切分（仍 ≤ maxChars），其余单元绝不跨条合并。
