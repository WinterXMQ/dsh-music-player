...
export const LIMITS = {
  ...
  itemsPerCategory: 8,
  totalItems: 20,
  summaryChars: 220, // 单条摘要硬上限：提示词严禁超限（200~220 字、提交前自查）；超长在此兜底——句末标点处断句，无标点才硬切省略号
  titleChars: 60,
...
/**
 * 摘要超限时的兜底裁剪：提示词要求 200~220 字并严禁超限，这里只兜底——超长时优先在
 * 上限内最后一个句末标点（。！？）处断句，保证播报不「断在半截」；整段没有任何句末
 * 标点才退回硬切 + 省略号。
 */
const clipSummary = (s, cap) => {
  const t = String(s === undefined || s === null ? '' : s).trim()
  if (t.length <= cap) return t
  const head = t.slice(0, cap)
  const cutAt = Math.max(head.lastIndexOf('。'), head.lastIndexOf('！'), head.lastIndexOf('？'))
  return cutAt >= 0 ? head.slice(0, cutAt + 1) : clip(t, cap)
}
...
export function sanitizeEditionInput(body, { today } = {}) {
  ...
  for (const c of rawCats) {
    ...
    for (const it of rawItems) {
      ...
      const summary = clipSummary(it.summary, LIMITS.summaryChars)
...
export function renderScript({ title, date, categories, opening, closing }) {
  ...
  // 开场：简洁播报台标与日期即可；具体类别由后面的「首先来听X」引导，避免重复啰嗦。
  if (opening) {
  ...
  // 类别与条目。**单类别期次不播「首先来听X」引导**——单主题班次的开场（如「以下是今日
  // 国内要闻。」）往往已点名类别，再接「首先来听国内。」明显重复；此时类别偏移改记在
  ...
  categories.forEach((cat, ci) => {
    ...
    cat.items.forEach((it) => {
      ...
      const sentence = `${seq === '第一条' ? '第一条' : seq}，${it.title}。${trimEndPunct(it.summary)}。`
