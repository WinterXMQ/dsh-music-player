...
export function apply(ctx) {
  ...
  const scan = async (rootPath) => {
    ...
    const walk = async (dir, depth) => {
      ...
      let entries = []
      try { entries = await ctx.fs.listDir(dir) } catch { return }
      for (const entry of entries) {
        if (found.length >= 500) return
        if (entry.type === 'directory') { await walk(entry.target, depth + 1); continue }
        if (entry.type !== 'file' || !isAudioName(entry.name)) continue
        const abs = ctx.fs.processPath(entry.target)
        const rel = abs.startsWith(rootStr) ? abs.slice(rootStr.length + 1) : entry.name
        found.push({
          name: rel, path: abs, size: entry.size || 0,
    ...
    try {
      ...
      if (pathname === '/dsh-music/dir' && req.method === 'GET') {
        ...
        try {
          ...
          let entries = []
          try { entries = await ctx.fs.listDir(target) } catch { entries = [] }
          const dirs = entries
            .filter((e) => e.type === 'directory')
            .map((e) => ({ name: e.name, path: ctx.fs.processPath(e.target) }))
            .sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0))
          writeJson(res, { path: abs, name: abs.slice(abs.lastIndexOf('/') + 1) || abs, dirs })
        } catch (err) {
  ...
  const tool = {
    name: 'music_play',
    description: '在 DSH 本地音乐库中搜索并播放音乐。给出歌曲名/歌手关键词，不传则播放第一首。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: { query: { type: 'string', description: '歌曲名/歌手关键词，可留空' } },
    },
    output: {
      schema: {
        type: 'object', additionalProperties: false,
        properties: { played: { type: 'boolean' }, track: { type: 'string' }, notice: { type: 'string' } },
      },
      render(_args, value) {
    ...
    async execute(args) {
      ...
      if (tracks.length === 0) {
        return {
          played: false, track: '',
          notice: '本地音乐库为空。请打开播放列表面板，点击「选择音乐目录」配置音乐目录。',
        }
      }
      const query = args && typeof args.query === 'string' ? args.query.trim().toLowerCase() : ''
      const list = query === '' ? tracks : tracks.filter((t) => t.name.toLowerCase().includes(query))
      if (list.length === 0) {
        return { played: false, track: '', notice: '没有找到包含「' + args.query + '」的音乐（音乐库共 ' + tracks.length + ' 首）。' }
      }
      const pick = list[0]
      pendingIntent = { id: pick.id, name: pick.name }
      return {
  ...
  ctx.systemPrompt.section({
    ...
    text: '本机已挂载本地音乐播放器：可用 music_play 工具按关键词播放 ~/Music（或设置的目录）里的音乐。',
  })
