...
import {
  ...
  sanitizeEditionInput, buildEdition, applyRetention, findInCooldown,
  summarizeEdition, metaForEdition, estimateMinutes,
...
export function apply(ctx) {
  ...
  //   editions[]      期次（结构化数据 + 口播稿分块，按任务分组滚动保留 7 期）
  //   schedulePrefs   面板「定时规则编辑器」的偏好（含 prefVersion/syncedVersion/model）
  //   runState        当前收集运行态（agent begin 上报，news_broadcast/reportFailure 清除）
  //   failures[]      最近收集失败日志（透传工具错误，≤10 条）
  //   newsSessionId   专用「新闻简报」会话 id（同步时自动创建/复用，承载每日定时）
  // 音频不落盘：TTS 懒合成（播放到哪块合成哪块），复用 synthesizeCached 内存缓存。
  let news = { editions: [], schedulePrefs: sanitizeSchedulePrefs({}), runState: null, failures: [], newsSessionId: null }
  ...
  const loadNews = async () => {
    ...
    news = {
      ...
      newsSessionId: data && typeof data.newsSessionId === 'string' && data.newsSessionId !== '' ? data.newsSessionId : null,
    }
    return news
  }
  const saveNews = async () => {
    ...
    try {
      ...
      writeFileSync(file, JSON.stringify({
        ...
        newsSessionId: news.newsSessionId,
      }, null, 2) + '\n', 'utf8')
  ...
  // 班次范围（scope）→ 人类可读摘要（工具输出/失败日志共用）。scope=null 表示继承默认。
  const scopeSummary = (scope) => {
    const s = scope || news.schedulePrefs.defaultScope
    const cats = (s.categories && s.categories.length > 0) ? s.categories : PRESET_CATEGORIES
    const topics = (s.topics && s.topics.length > 0) ? s.topics : []
    return topics.length > 0 ? `${cats.join('/')} + 主题:${topics.join(',')}` : cats.join('/')
  }

  // ---- 自动化通道：把指令作为 user 消息注入 agent（镜像 harness schedule 插件的官方模式）----
  // schedule 插件到点投递提醒用的就是「构造 user 消息 + agent.followup()」——公开接口，
  // 非内部格式。新闻插件的「同步定时 / 立即执行」走同一通道：面板一键触发，agent 被
  // 唤醒后按 news_schedule 的指引自动创建/更新定时任务或立即收集，用户无需打字。
  // 目标会话：优先「专用新闻简报会话」（没有则自动创建，用户无需手动开会话），
  // 其次「正在运行」的 root agent（用户正在交互的会话），否则取注册序最后一个。
  // ctx.agents 用懒获取（不加 inject 硬依赖）：服务缺失时返回 null，调用方优雅回退到
  // 「复制指令」路径。
  //
  // 专用会话：同步/立即执行时自动创建或复用。模型来源：面板定时偏好 schedulePrefs.model
  // （用户在选择器里选），否则复制最近一个活跃 root 会话的模型；cwd 沿用来源会话（无则空）。
  // 创建失败 / 无 agents 服务 / 无模型可建 → 返回 null，调用方优雅回退。
  const ensureNewsSession = async () => {
    ...
    if (news.newsSessionId) {
      const live = agentsSvc.get(news.newsSessionId)
      if (live && live.session && typeof live.followup === 'function') return live
      // 记录的 id 已不在（会话被关 / 宿主重启未恢复）→ 清掉，重新创建。
      news.newsSessionId = null
    }
    ...
    if (!provider || !model) return null // 没有模型可建，回退
    const cwd = (source && source.session && source.session.header && source.session.header.cwd) || (await getHome()) || null
    const sessionId = 'news-briefing-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 8)
  ...
  const injectNewsInstruction = async (text, { createSession = true } = {}) => {
    const agentsSvc = ctx.get ? ctx.get('agents') : null
    if (!agentsSvc || typeof agentsSvc.roots !== 'function') return null
    let dedicated = null
    if (createSession) {
      dedicated = await ensureNewsSession()
    } else if (news.newsSessionId) {
      // 不新建（如停用清理场景）：仅复用一个已存在的专用会话，否则回退当前活跃会话。
      const live = agentsSvc.get ? agentsSvc.get(news.newsSessionId) : null
      if (live && live.session && typeof live.followup === 'function') dedicated = live
    }
    const roots = agentsSvc.roots().filter((a) => a && a.session && typeof a.followup === 'function')
    if (roots.length === 0) return null
    const target = dedicated && dedicated.session && typeof dedicated.followup === 'function'
      ? dedicated
      : (roots.find((a) => a.status === 'running') || roots[roots.length - 1])
    const message = {
  ...
  const serve = async (req, res) => {
    try {
      ...
      //   DELETE /dsh-music/news/<id>           -> 删除某期
      //   POST   /dsh-music/news/play {id}      -> 推送播放（面板按钮）
      //   POST   /dsh-music/news/played {id}    -> 标记已播（清除「待播」徽标）
      //   GET    /dsh-music/news/runstate       -> 当前收集运行态（begin/reportFailure 维护）
      //   GET/POST /dsh-music/news/schedule     -> 定时偏好读写（面板定时规则编辑器）
      //   POST   /dsh-music/news/schedule/synced {version} -> agent 回写同步版本号
      if (pathname === '/dsh-music/news' && req.method === 'GET') {
        await loadNews()
        writeJson(res, { ok: true, editions: news.editions.map(summarizeEdition) })
        return
      }
      ...
      // 供面板「新闻会话模型」选择器：列出可用 provider 及各 provider 的模型。
      ...
      if (pathname === '/dsh-music/news/schedule' && (req.method === 'GET' || req.method === 'POST')) {
        await loadNews()
        if (req.method === 'GET') {
          writeJson(res, { ok: true, schedulePrefs: news.schedulePrefs, failures: news.failures.slice(-10), newsSessionId: news.newsSessionId })
          return
        }
        const body = await readBody(req)
        ...
        const changed = JSON.stringify({ e: next.enabled, d: next.defaultScope, s: next.shifts })
          !== JSON.stringify({ e: prev.enabled, d: prev.defaultScope, s: prev.shifts })
      ...
      if (pathname === '/dsh-music/news/schedule/synced' && req.method === 'POST') {
        await loadNews()
        const body = await readBody(req)
        const version = Number(body && body.version)
        news.schedulePrefs = sanitizeSchedulePrefs({}, {
          ...news.schedulePrefs,
          syncedVersion: Number.isFinite(version) ? Math.trunc(version) : news.schedulePrefs.prefVersion,
        })
        await saveNews()
        writeJson(res, { ok: true, schedulePrefs: news.schedulePrefs })
        return
      }
      // 自动同步：把同步指令注入 agent（followup 唤醒），agent 按 news_schedule 的指引
      // 自动创建/更新/删除 DSH 定时任务并 markSynced。agents 服务缺失时返回
      // { ok:false, fallback:true }，客户端回退「复制指令」路径。
      if (pathname === '/dsh-music/news/schedule/sync' && req.method === 'POST') {
        await loadNews()
        const p = news.schedulePrefs
        if (!p.enabled || (p.shifts || []).length === 0) {
          // 停用/无班次：注入「删除全部新闻定时任务」的指令
          const target = await injectNewsInstruction('同步新闻定时：每日定时已在我的面板停用（无班次）。你是专用「新闻简报」会话。请删除当前会话里全部新闻播报相关的定时任务（schedule_list 查询后逐条 schedule_delete），完成后调用 news_schedule {action:"markSynced"} 回写版本号并向我汇报结果。', { createSession: false })
          if (target === null) { writeJson(res, { ok: false, fallback: true, error: 'agents 服务不可用' }); return }
          writeJson(res, { ok: true, target: target.id, mode: 'delete-all' })
          return
        }
        const target = await injectNewsInstruction('同步新闻定时：我的面板定时偏好有修改未同步。你是专用「新闻简报」会话。请调用 news_schedule {action:"get"} 获取偏好与同步指引，然后创建/更新/删除对应的 DSH 定时任务，完成后调用 news_schedule {action:"markSynced", version:<prefVersion>} 回写版本号，并向我汇报创建结果。')
        if (target === null) { writeJson(res, { ok: false, fallback: true, error: 'agents 服务不可用' }); return }
        writeJson(res, { ok: true, target: target.id, mode: 'sync' })
        return
      }
      // 立即执行班次：把执行指令注入 agent（不等时刻、不需要用户打字）。
      if (pathname === '/dsh-music/news/run-now' && req.method === 'POST') {
        ...
        const scopeText = scopeSummary(shift.scope)
        const playText = shift.autoplay === false ? '收集完成后先不播放（静默收集）' : '收集完成后立即用 news_broadcast 提交并自动播放'
        const text = `立即执行我的 ${shift.time} 新闻班次（班次 id ${shift.id}）：收集${scopeText}相关的今日头条，整理后提交播报，${playText}。请先调用 news_schedule {action:"begin", shiftId:"${shift.id}"} 上报开始，再按收集流程执行。`
        const target = await injectNewsInstruction(text)
        if (target === null) { writeJson(res, { ok: false, fallback: true, error: 'agents 服务不可用' }); return }
        writeJson(res, { ok: true, target: target.id })
        return
      }
  ...
  const newsBroadcastTool = {
    name: 'news_broadcast',
    description: '把收集整理好的当日新闻提交为语音播报。调用前必须已用 web_search 完成收集与筛选（按类别多查询、带当天日期、跨源去重、只保留可确认时效的条目）。每条 item 的 summary 为 ≤80 字的中性口播摘要，source 必填（如「新华社」「微博热搜」）。同一班次 10 分钟内重复提交会被跳过（冷却窗），确需强制重收传 force:true。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        title: { type: 'string', description: '期次标题，如「早间新闻播报」「AI 新闻简报」，默认「今日新闻播报」' },
        ...
        categories: {
          ...
          items: {
            ...
            additionalProperties: false,
            properties: {
              name: { type: 'string', description: '类别名（预设：热点/国内/国际/科技/财经/体育；自定义主题直接用主题名，如 AI）' },
              items: {
                type: 'array',
                items: {
                  type: 'object',
                  ...
                  properties: {
                    ...
                    summary: { type: 'string', description: '≤80 字中性口播摘要' },
    ...
    async execute(args) {
      ...
      // 收集完成即清除运行态：同一会话内 agent 串行执行，任一收集结束都意味着
      // 当前运行结束（手动/定时、有无 shiftId 均适用）。
      news.runState = null
      await saveNews()
  ...
  const newsScheduleTool = {
    name: 'news_schedule',
    description: '查询新闻播报的定时偏好与状态、上报收集开始/失败、回写同步版本。用户要求「同步新闻定时」时：先 get 读取面板偏好，再用 schedule_create/schedule_delete 使实际定时任务与偏好一致（每班次一条 every:86400、scheduledAt 锚定班次时刻；提醒内容必须写明收集范围与是否静默），完成后 markSynced 回写偏好版本号。开始一轮收集时先 begin 上报（面板显示执行中）；收集失败（换词重试后仍无结果或工具报错）时 reportFailure 透传错误。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
        action: { type: 'string', enum: ['get', 'begin', 'markSynced', 'reportFailure'], description: 'get 查询偏好与状态；begin 上报开始收集；markSynced 回写已同步版本号；reportFailure 上报收集失败' },
        shiftId: { type: 'string', description: '班次 id（begin/reportFailure 时填）' },
        version: { type: 'number', description: '偏好版本号（markSynced 时填偏好返回的 prefVersion）' },
    ...
    async execute(args) {
      ...
      if (action === 'begin') {
        news.runState = { shiftId: shiftId || null, startedAt: Date.now(), scope: shiftId ? scopeSummary(news.schedulePrefs.shifts.find((s) => s.id === shiftId)?.scope ?? null) : '' }
        await saveNews()
        return { ok: true, data: '', notice: '已上报开始收集' + (shiftId ? `（班次 ${shiftId}）` : '') + '。' }
      }
      if (action === 'markSynced') {
        const version = Number(args && args.version)
        news.schedulePrefs = sanitizeSchedulePrefs({}, { ...news.schedulePrefs, syncedVersion: Number.isFinite(version) ? Math.trunc(version) : news.schedulePrefs.prefVersion })
        await saveNews()
        return { ok: true, data: '', notice: `已回写同步版本号（${news.schedulePrefs.syncedVersion}）。` }
      }
      if (action === 'reportFailure') {
        const kind = args && args.kind === 'empty' ? 'empty' : 'error'
        const reason = String((args && args.reason) || '').slice(0, 160)
        pushFailure({ shiftId: shiftId || null, kind, reason })
        news.runState = null // 同上：任一收集失败也结束当前运行
        await saveNews()
      ...
      const desc = {
        ...
        model: p.model, // 用户选择的「新闻会话模型」（null = 跟随当前活跃会话）
        newsSessionId: news.newsSessionId, // 专用「新闻简报」会话 id（null = 尚未创建）
        inSync: p.prefVersion === p.syncedVersion || (p.shifts.length === 0 && p.prefVersion === 0),
        running: runStateAlive(news.runState, Date.now()) ? news.runState : null,
        recentFailures: news.failures.slice(-5),
        notice: !p.enabled
          ? '每日定时已停用；用户要求同步时删除全部新闻定时任务并 markSynced。'
          : p.shifts.length === 0 && p.prefVersion === 0
            ? '面板尚未配置定时班次。'
            : p.prefVersion === p.syncedVersion
              ? '定时偏好与已同步版本一致。'
              : `面板定时偏好有修改未同步（偏好 v${p.prefVersion} / 已同步 v${p.syncedVersion}）。用户要求同步时，请按 shifts 逐条创建 schedule_create（everySeconds: 86400，scheduledAt 取最近的班次时刻并注明 IANA 时区；autoplay=false 的班次提醒内容注明「静默收集，不自动播放」），多余/过期的旧定时用 schedule_delete 删除，完成后用 markSynced 回写 prefVersion。`,
      }
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '1) 先 news_schedule {action:"begin", shiftId} 上报开始（定时班次触发时必带 shiftId）；',
      '2) 用 web_search 按类别多查询（每批 1~4 条、全部带当天日期锚定，如「5月30日 热搜榜」「5月30日 国内要闻 新华社」）；类别来源优先级：热点→微博热搜/百度热搜/知乎热榜聚合报道（按热度排序、标榜单位）；国内→新华社/人民日报/央视/澎湃；国际→参考消息/环球网/CGTN；科技→36氪/IT之家；财经→财新/第一财经；体育→央视体育/懂球帝；自定义主题（如 AI）→ 主题词组合查询并优先该主题权威垂直媒体（AI→机器之心/量子位）；',
      '3) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条；每类 ≤8 条、全期 ≤20 条；每条写 ≤80 字中性口播摘要并必标 source，不做猜测性推断；',
      ...
      '6) 用户要求创建/修改/取消新闻定时任务（如「每天早上9点播AI新闻」）：必须把用户指定的范围（类别/主题/是否播放/条数）显式写进 schedule_create 的提醒内容——提醒内容是一份完整的到点执行指令（按范围搜索→去重整理→news_broadcast 提交，autoplay 按用户要求；每班次一条，everySeconds: 86400、scheduledAt 取最近一次该时刻并注明 IANA 时区）；若面板已配置定时偏好（news_schedule {action:"get"} 能查到 shifts 且 prefVersion>0），以偏好为准逐条创建/删除使实际定时与偏好一致，完成后 markSynced 回写版本号；取消用 schedule_delete。你是专用「新闻简报」会话，新闻定时任务建在本会话（面板同步会自动创建/复用本会话，无需用户手动开）。',
    ].join('\n'),
  })
