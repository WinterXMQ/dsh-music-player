...
import {
  ...
  sanitizeSchedulePrefs, runStateAlive,
} from './news-core.js'
...
export function apply(ctx) {
  ...
  //   schedulePrefs   面板「定时规则编辑器」的偏好（含 prefVersion/syncedVersion）
  ...
  // 目标会话：优先「正在运行」的 root agent（用户正在交互的会话），否则取注册序最后一个。
  // ctx.agents 用懒获取（不加 inject 硬依赖）：服务缺失时返回 null，调用方优雅回退到
  // 「复制指令」路径。
  const injectNewsInstruction = (text) => {
    ...
    const target = roots.find((a) => a.status === 'running') || roots[roots.length - 1]
    const message = {
    ...
    try {
      ...
      if (pathname === '/dsh-music/news/schedule/sync' && req.method === 'POST') {
        ...
        if (!p.enabled || (p.shifts || []).length === 0) {
          ...
          const target = injectNewsInstruction('同步新闻定时：每日定时已在我的面板停用（无班次）。请删除当前会话里全部新闻播报相关的定时任务（schedule_list 查询后逐条 schedule_delete），完成后调用 news_schedule {action:"markSynced"} 回写版本号并向我汇报结果。')
        ...
        const target = injectNewsInstruction('同步新闻定时：我的面板定时偏好有修改未同步。请调用 news_schedule {action:"get"} 获取偏好与同步指引，然后创建/更新/删除对应的 DSH 定时任务，完成后调用 news_schedule {action:"markSynced", version:<prefVersion>} 回写版本号，并向我汇报创建结果。')
        if (target === null) { writeJson(res, { ok: false, fallback: true, error: 'agents 服务不可用' }); return }
      ...
      if (pathname === '/dsh-music/news/run-now' && req.method === 'POST') {
        ...
        const target = injectNewsInstruction(text)
        if (target === null) { writeJson(res, { ok: false, fallback: true, error: 'agents 服务不可用' }); return }
