...
export function apply(ctx) {
  ...
  // 销毁一个执行会话：优先用本进程内存表里持有的句柄；否则（跨重启后句柄丢失）尽力
  // resume→dispose 兜底。DSH 的 agents 服务只允许「创建者通过句柄 dispose」、
  // sessionPersistence 只追加不删除，所以跨重启后唯一拿到销毁能力的方式是 resume 旧会话
  // 拿到新句柄后立即 dispose——resume 会短暂拉起旧会话循环，但 dispose() 立即以 disposed
  // 原因取消、注销并从存储移除该会话。任何失败都吞掉（best-effort，不阻断删除期次）。
  const disposeExecutionSession = async (sessionId) => {
    ...
    try {
      const agentsSvc = ctx.get ? ctx.get('agents') : null
      if (!agentsSvc || typeof agentsSvc.resume !== 'function') return
      const m = deriveAgentModel()
      if (!m) return
  ...
  const runCollection = async (shiftId) => {
    ...
    const text = (shiftId
      ? `立即执行我的 ${shift.time} 新闻班次（班次 id ${shift.id}）：收集${scopeText}相关的今日头条，整理后提交播报，${playText}。请按新闻收集流程执行。`
      : `请收集今日头条并按新闻收集流程执行，${playText}。`)
  ...
  const serve = async (req, res) => {
    try {
      ...
      //   GET    /dsh-music/news/models         -> 可用 provider/model（新闻会话模型选择器）
      ...
      if (pathname === '/dsh-music/news/run-now' && req.method === 'POST') {
        ...
        if (!r.ok) { writeJson(res, { ok: false, fallback: true, error: r.error }); return }
  ...
  const newsBroadcastTool = {
    name: 'news_broadcast',
    description: '把收集整理好的当日新闻提交为语音播报。调用前必须已用 web_search 完成收集与筛选（按类别多查询、带当天日期、跨源去重、只保留可确认时效的条目）。每条 item 的 summary 写充实些（120~260 字的中性口播内容，讲清事件要点、背景与影响，别只写一句），source 必填（如「新华社」「微博热搜」）。同一班次 10 分钟内重复提交会被跳过（冷却窗），确需强制重收传 force:true。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
              ...
              items: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    summary: { type: 'string', description: '120~260 字的中性口播内容（讲清事件要点、背景与影响）' },
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '1) 用 web_search 按类别多查询（每批 1~4 条、全部带当天日期锚定，如「5月30日 热搜榜」「5月30日 国内要闻 新华社」）；类别来源优先级：热点→微博热搜/百度热搜/知乎热榜聚合报道（按热度排序、标榜单位）；国内→新华社/人民日报/央视/澎湃；国际→参考消息/环球网/CGTN；科技→36氪/IT之家；财经→财新/第一财经；体育→央视体育/懂球帝；自定义主题（如 AI）→ 主题词组合查询并优先该主题权威垂直媒体（AI→机器之心/量子位）；',
      '2) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条；每类 ≤8 条、全期 ≤20 条；每条写充实些（120~260 字中性口播内容，讲清事件要点、背景与影响，别只写一句）并必标 source，不做猜测性推断；',
      '3) 调 news_broadcast 提交（用户没说「先别播」时 autoplay 默认 true；静默收集传 false）；同一班次 10 分钟内重复提交会被冷却窗跳过；',
      '4) 失败处理：搜索成功但无结果→换查询词重试 2~3 次，仍无果调 news_schedule {action:"reportFailure", kind:"empty"} 并告知用户；工具调用报错→确认性重试 1 次即放弃，reportFailure 透传错误码与消息原文（不要推断原因），本期次不生成、绝不用旧数据顶替；',
      '5) 定时由 Host 端自维护：用户在面板「⏰ 每日定时」配置班次（时刻/范围/是否立即播放/新闻会话模型）即生效，到点 Host 自动新建一个执行会话去收集并绑定结果，无需 agent 创建/同步 DSH 定时任务；用户要求查看/修改/取消定时时，引导其到面板操作即可。',
    ].join('\n'),
  })
