    ...
    try {
      ...
      if (pathname.startsWith('/dsh-music/news/')) {
        ...
        if (req.method === 'DELETE') {
          ...
          if (removed) {
            // 删除联动：销毁该期次对应的执行会话（若仍存活），并清映射。
            if (hit && hit.sessionId && execSessions.has(hit.sessionId)) {
              const rec = execSessions.get(hit.sessionId)
              if (rec && rec.handle && typeof rec.handle.dispose === 'function') {
                try { await rec.handle.dispose() } catch {}
              }
              execSessions.delete(hit.sessionId)
            }
            await saveNews()
          }
