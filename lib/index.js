...
export function apply(ctx) {
  ...
  const runCollection = async (shiftId) => {
    ...
    const text = (shiftId
      ? `立即执行我的 ${shift.time} 新闻班次（班次 id ${shift.id}）：${nowText}。请收集${scopeText}相关的今日头条——搜索查询词一律用上面的日期锚定当天（如「${nowD.getMonth() + 1}月${nowD.getDate()}日 热搜榜」），整理后提交播报，${playText}。请按新闻收集流程执行。`
  ...
  const newsBroadcastTool = {
    name: 'news_broadcast',
    description: '把收集整理好的当日新闻提交为语音播报。调用前必须已用 web_search 完成收集与筛选（按类别多查询、带当天日期、跨源去重、只保留可确认时效的条目）。每条 item 的 summary 必须写在 200~220 字之间，这是硬性要求：严禁超过 220 字（超出会被截断、句子断在半截，播报很难听），也不得少于 200 字（信息量不足）；提交前逐条数字数自查，接近 220 就用完整句子自然收尾。source 必填（如「新华社」「微博热搜」）。同一班次 10 分钟内重复提交会被跳过（冷却窗），确需强制重收传 force:true。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
              ...
              items: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    summary: { type: 'string', description: '200~220 字的中性口播内容；严禁超过 220 字、不得少于 200 字，提交前逐条自查字数' },
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '2) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条；每类 ≤8 条、全期 ≤20 条；每条摘要必须写在 200~220 字之间，这是硬性要求——严禁超过 220 字（超出一经系统截断句子就断在半截，播报很难听），也不得少于 200 字（信息量不足）；写完逐条数字数自查，接近 220 就用完整句子自然收尾，绝不依赖系统截断；摘要为中性口播内容，讲清事件要点、背景与影响，必标 source，不做猜测性推断；',
      ...
      '6) 定时由 Host 端自维护：用户在面板「⏰ 每日定时」配置班次（时刻/范围/是否立即播放/新闻会话模型）即生效，到点 Host 自动新建一个执行会话去收集并绑定结果，无需 agent 创建/同步 DSH 定时任务；用户要求查看/修改/取消定时时，引导其到面板操作即可。',
    ].join('\n'),
  })
