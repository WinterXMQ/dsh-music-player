...
export function apply(ctx) {
  ...
  const newsBroadcastTool = {
    name: 'news_broadcast',
    description: '把收集整理好的当日新闻提交为语音播报。调用前必须已用 web_search 完成收集与筛选（按类别多查询、带当天日期、跨源去重、只保留可确认时效的条目）。每条 item 的 summary 写充实些（150~450 字的中性口播内容，讲清事件要点、背景与影响，完整成段、别只写一句），source 必填（如「新华社」「微博热搜」）。同一班次 10 分钟内重复提交会被跳过（冷却窗），确需强制重收传 force:true。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
              ...
              items: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    summary: { type: 'string', description: '150~450 字的中性口播内容（讲清事件要点、背景与影响，完整成段）' },
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '2) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条；每类 ≤8 条、全期 ≤20 条；每条写充实些（150~450 字中性口播内容，讲清事件要点、背景与影响，完整成段）并必标 source，不做猜测性推断；',
