...
export function apply(ctx) {
  ...
  const PREF_ALLOW = new Set([
    ...
    'dsh-music-panel-pos', 'dsh-music-playback', 'dsh-music-qq-playback', 'dsh-music-books-playback',
  ...
  const saveKGCookie = async () => {
    ...
    try {
      mkdirSync(dirname(file), { recursive: true })
      writeFileSync(file, JSON.stringify({ session: kg.session, loggedIn: kg.loggedIn, savedAt: Date.now() }), { encoding: 'utf8', mode: 0o600 })
    } catch { /* best-effort */ }
  }
  ...
  const kgWithFreshToken = async (fn) => {
    try { return await fn() } catch (e) {
      ...
      if (!/20017|20018|设备不匹配/.test(m) || !kg.loggedIn) throw e
  ...
  const clearKGCookie = async () => {
    kg.session = { guid: '', mid: '', dfid: '-', token: '', userid: '', vip_type: '', vip_token: '' }
    kg.loggedIn = false; kg.isVip = null
    kgTakeCacheKg.clear()
    const file = await kgCookieFile()
    if (file !== null) { try { writeFileSync(file, JSON.stringify({ session: kg.session, loggedIn: false }), { encoding: 'utf8', mode: 0o600 }) } catch { /* ignore */ } }
  }
  ...
  const serve = async (req, res) => {
    try {
      ...
      if (pathname === '/dsh-music/kg/login/start' && req.method === 'POST') {
        await loadKGCookie()
        try {
          // 关键：每次出码都重建完整设备身份并注册真实 dfid（r_register_dev）。
          // 酷狗把「token ↔ 设备」绑定校验：伪造 dfid 上出码的 token 会在取链(20028
      ...
      if (pathname === '/dsh-music/kg/playlists' && req.method === 'GET') {
        ...
        try {
          if (category === '') {
            ...
            const page = Math.max(1, parseInt(url.searchParams.get('page') || '1', 10) || 1)
            writeJson(res, { ok: true, playlists: await KG.getCategoryPlaylists(category, page, 20) })
          }
        } catch (err) { writeJson(res, { ok: false, error: String((err && err.message) || err) }, 502) }
        return
      }
      if (pathname === '/dsh-music/kg/playlist-categories' && req.method === 'GET') {
        try { writeJson(res, { ok: true, categories: await KG.getPlaylistCategories() }) }
        catch (err) { writeJson(res, { ok: false, error: String((err && err.message) || err) }, 502) }
        return
      }
      ...
      if (pathname.startsWith('/dsh-music/kg/my-playlist/') && req.method === 'GET') {
        ...
        if (!kg.loggedIn) { writeJson(res, { ok: false, error: '未登录' }, 401); return }
        try {
          const songs = await kgWithFreshToken(() => KG.getMyPlaylistSongs(id, kg.session))
