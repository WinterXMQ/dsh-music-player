...
export function apply(ctx) {
  ...
  const tool = {
    name: 'music_play',
    description: '控制 DSH 本地音乐库与小说库的播放。播放时可按歌曲名/歌手/小说名关键词搜索并播放（不传 query 则播放第一首音乐，没有音乐则播放第一本小说）；也可用 action 执行暂停/继续/停止/下一首·下一章/上一首·上一章。',
    parameters: {
      type: 'object',
    ...
    async execute(args) {
      ...
      if (query === '') {
        ...
        return {
          ...
          notice: '已请求播放小说「' + pick.name + '」。讲书需已配置 xiaomi/MiMo TTS；若被拦截请点 ▶ 解锁。',
        }
      }
      ...
      if (bookPool.length > 0 && musicPool.length === 0) {
        ...
        return {
          ...
          notice: '已请求播放小说「' + pick.name + '」（匹配 ' + bookPool.length + ' / 共 ' + bookCount + ' 本）。讲书需已配置 xiaomi/MiMo TTS；若被拦截请点 ▶ 解锁。',
        }
      }
  ...
  ctx.systemPrompt.section({
    ...
    text: '本机已挂载本地音乐播放器与 AI 讲书：可用 music_play 工具按关键词播放 ~/Music（或设置的目录）里的音乐，或按小说名播放本地 .txt 小说（AI 讲书，需配置 xiaomi/MiMo TTS）；并支持 action 暂停/继续/停止/下一首·下一章/上一首·上一章。',
  })
