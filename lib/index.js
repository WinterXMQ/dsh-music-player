...
export function apply(ctx) {
  ...
  const synthesize = async (text) => {
    ...
    const body = {
      ...
      audio: { format: 'wav', voice: MIMO_DEFAULT_VOICE },
      stream: false,
    }
  ...
  const synthesizeCached = async (cacheKey, text) => {
  ...
  const splitBookChunks = (text) => {
    const chunks = []
    const sentences = text.split(/(?<=[。！？；…])/u)
    let cur = ''
    for (const part of sentences) {
      const sentence = part.replace(/\s*\n+\s*/g, ' ').trim()
      if (sentence.length === 0) continue
      if (cur.length > 0 && cur.length + sentence.length > MAX_TTS_CHARS) {
        chunks.push(cur)
        cur = ''
      }
  ...
  // the file on every block request.
  const bookChunksCache = {}
  const loadBookChunks = (absPath) => {
    if (bookChunksCache[absPath] === undefined) {
      bookChunksCache[absPath] = splitBookChunks(readBookText(absPath))
    }
    ...
    try {
      ...
      if (pathname === '/dsh-music/manifest' && req.method === 'GET') {
        ...
        writeJson(res, {
          ...
          ttsConfigured: ts.configured, ttsReason: ts.reason,
        })
        return
      }
      ...
      if (pathname.startsWith('/dsh-music/book/') && (req.method === 'GET' || req.method === 'HEAD')) {
        ...
        try {
          ...
          const chunks = loadBookChunks(book.path)
          if (isMeta) {
            writeJson(res, { id, name: book.name, total: chunks.length, size: book.size })
            return
          }
          ...
          const wav = await synthesizeCached(book.id + ':' + idx, chunks[idx])
          const headers = {
