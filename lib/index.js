...
export function apply(ctx) {
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '6) 用户要求创建/修改/取消新闻定时任务：优先引导已配置面板偏好时用 news_schedule {action:"get"} 读取并按其 shifts 逐条 schedule_create（everySeconds:86400、scheduledAt 为最近一次班次时刻含 IANA 时区，提醒内容写明收集范围与是否静默）；取消则 schedule_delete；同步完成后 markSynced 回写版本号。新闻定时建议建在专用「新闻简报」会话。',
    ].join('\n'),
  })
