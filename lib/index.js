...
export function apply(ctx) {
  ...
  //    ctx.workspace.archiveSession(id)：归档集合持久化在 storage domain，重启不丢，
  ...
  const disposeExecutionSession = async (sessionId) => {
    ...
    try {
      const ws = ctx.get ? ctx.get('workspace') : null
  ...
  const newsBroadcastTool = {
    name: 'news_broadcast',
    description: '把收集整理好的当日新闻提交为语音播报。调用前必须已用 web_search 完成收集与筛选（按类别多查询、带当天日期、跨源去重、只保留可确认时效的条目）。每条 item 的 summary 字数要明确控制在 150~280 字（中性口播内容，讲清事件要点、背景与影响；超过 280 字会被系统截断，务必在此范围内写完整），source 必填（如「新华社」「微博热搜」）。同一班次 10 分钟内重复提交会被跳过（冷却窗），确需强制重收传 force:true。',
    parameters: {
      type: 'object',
      additionalProperties: false,
      properties: {
              ...
              items: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    summary: { type: 'string', description: '150~280 字的中性口播内容（超过 280 字会被截断，务必在此范围内写完整）' },
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '2) 质量控制：只保留可确认时效的条目（publishedAt 或正文时间线索，存疑即弃）；同一事件跨源去重只留最权威一条；每类 ≤8 条、全期 ≤20 条；每条摘要字数严格控制在 150~280 字（中性口播内容，讲清事件要点、背景与影响；超过 280 字会被系统截断，务必在范围内写完整）并必标 source，不做猜测性推断；',
