...
const emStrip = (s) => dec(String(s ?? '').replace(/<\/?em>/g, ''))
...
function normalizeSong(o, opts = {}) {
  ...
  const arts = Array.isArray(o.authors)
    ...
    : (() => {
        ...
        return String(raw).split(/、|\/|,/).map((x) => emStrip(x.trim())).filter(Boolean)
      })()
  let title = emStrip(o.songname != null && o.songname !== '' ? o.songname : (o.SongName ?? ''))
  if (title === '' && o.filename) {
    const sp = splitFileName(o.filename)
    title = sp.title
    if (arts.length === 0) arts.splice(0, 0, ...sp.artists)
  }
...
export async function getTopListSongs(rankId, _session = {}, offset = 0, num = 30) {
  ...
  const total = Math.max(Number(j.data.count) || songs.length + off, songs.length + off)
  return {
    id: rankid,
...
/** 歌单分类标签（两级：组 → children）。 */
export async function getPlaylistCategories(_session = {}) {
  const res = await fetchWithTimeout('http://mobilecdnbj.kugou.com/api/v3/tag/list?pid=0&apiver=2&plat=0', {
    headers: { 'User-Agent': UA_WEB, Referer: 'http://m.kugou.com/' },
  })
  const j = await res.json().catch(() => null)
  if (!j || Number(j.status) !== 1 || !j.data) throw new Error('酷狗分类获取失败')
  const cats = []
  for (const g of j.data.info || []) {
    const group = emStrip(g.name)
    for (const c of (Array.isArray(g.children) ? g.children : [])) {
      if (c.id == null || !c.name) continue
      cats.push({ id: String(c.id), name: emStrip(c.name), group })
    }
  }
  return cats
}
...
/** 分类歌单（tagid 分页）。 */
export async function getCategoryPlaylists(tagId, page = 1, limit = 20, _session = {}) {
  const p = Math.max(1, parseInt(page, 10) || 1)
...
export async function getPlaylistSongs(specialId, _session = {}) {
  ...
  return {
    ...
    trackCount: Number(j.data.count) || songs.length,
    source: 'kugou',
    link: PL_LINK(sid),
    songs,
  }
...
const QUALITY_LABELS = { flac: '无损', high: 'Hi-Res', 320: '320kbps', 128: '128kbps', super: '超品', multitrack: '分轨' }
