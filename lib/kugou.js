...
// 「默认收藏」（is_def=1）云歌单接口不返回 pic 封面，用歌单第一首歌的封面兜底。
// 缓存 key = `${userid}:${listid}`，避免每次刷新「我的歌单」都重复拉取歌单歌曲。
const defCollectCoverCache = new Map()
...
export async function getMyPlaylists(session = {}, page = 1, pagesize = 60) {
  ...
  // 「默认收藏」（is_def=1）没有 pic 封面：取歌单第一首歌的封面兜底（带缓存）。
  const defItem = items.find((x) => x.kind === 'own' && x.isDef === 1 && !x.cover && x.id)
  if (defItem) {
    const key = String(s.userid || '') + ':' + String(defItem.id)
    let cover = defCollectCoverCache.get(key)
    if (cover === undefined) {
      cover = ''
      try {
        const songs = await getMyPlaylistSongs(defItem.id, s)
        if (songs && songs.length && songs[0].cover) cover = songs[0].cover
      } catch { /* 失败保持空封面（前端显示音符占位） */ }
      if (cover) defCollectCoverCache.set(key, cover)
    }
    if (cover) defItem.cover = cover
  }
  return items
}
