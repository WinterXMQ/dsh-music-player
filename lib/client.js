...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function NowPlayingBar() {
      ...
      // 自建歌单：范围徽标（来源即范围）与收藏爱心按钮。
      let scopeBadge = null;
      if (!isBook && store.scope && store.scope.kind === 'playlist') {
        const pl = playlistById(store.scope.id);
        if (pl) {
          scopeBadge = React.createElement('span', { className: 'dsh-music-bar-scope', title: '当前播放范围：歌单「' + pl.name + '」' },
            '歌单 · ' + pl.name,
            React.createElement('button', { className: 'dsh-music-bar-scope-x', title: '切回全部曲库', onClick: () => set({ scope: { kind: 'library' } }) }, '\u2715'),
          );
        }
      }
    ...
    function PlayerPanel() {
      ...
      // 音乐页子标签：曲库 / 我最喜欢 / ＋ / 自建歌单
      const subTabBtn = (key, label, extraCls) => React.createElement('button', {
      ...
      const musicSubTabs = React.createElement('div', { className: 'dsh-music-subtabs' },
        ...
        (s.playlists || []).filter((p) => p.id !== FAV_PLAYLIST_ID).map((p) => subTabBtn(p.id, p.name)),
      );
    ...
    // 歌单详情：歌单名 + 重命名/删除 + 添加歌曲 + 歌曲列表（移除/上移/下移）。
    function PlaylistDetail({ pl }) {
      ...
      return React.createElement('div', { className: 'dsh-music-playlist' },
        React.createElement('div', { className: 'dsh-music-playlist-head' },
          React.createElement('span', { className: 'dsh-music-playlist-title', title: pl.name }, pl.name),
        ...
        React.createElement('div', { className: 'dsh-music-playlist-addrow' },
          React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => setPickerOpen(true) }, '\uff0b \u6dfb\u52a0\u6b4c\u66f2')),
        rows.length > 0 ? rows : React.createElement('div', { className: 'dsh-music-empty' }, '歌单为空，点击「添加歌曲」从本地文件选择音乐。'),
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 自建歌单：音乐页子标签 / 歌单详情 / 文件多选 / 播放条收藏与范围徽标
      ...
      '.dsh-music-playlist-title { font-weight: 600; font-size: 13px; margin-right: auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }\n' +
      ...
      '.dsh-music-playlist-missing { flex: none; font-size: 11px; color: var(--dsw-alias-state-warn-primary, #d9a441); }\n' +
      '.dsh-music-playlist-addrow { padding: 6px 2px 2px; }\n' +
      ...
      '.dsh-music-bar-scope { margin-left: 8px; display: inline-flex; align-items: center; gap: 4px; flex: none; font-size: 11px; color: var(--dsw-alias-label-secondary, #8a8f98); border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.25)); border-radius: 6px; padding: 0 6px; line-height: 16px; }\n' +
      '.dsh-music-bar-scope-x { background: transparent; border: none; color: inherit; cursor: pointer; font-size: 11px; padding: 0; line-height: 1; }\n' +
      '.dsh-music-bar-scope-x:hover { color: var(--dsw-alias-state-error-primary, #e5534b); }\n' +
      '.dsh-music-bar-btn.fav { color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
      '.dsh-music-bar-btn.fav.on { color: var(--dsw-alias-state-error-primary, #e5534b); }\n';
