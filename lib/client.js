...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function restoreLatest(list) {
      ...
      if (kgTs >= 0 && scopeKind === 'kg') { restoreKGPlayback(); return; }
      if (qqTs >= 0 && scopeKind === 'qq') { restoreQQPlayback(); return; }
      if (book !== null && scopeKind === 'book') { restoreBookPlayback(); return; }
    ...
    function KGOnlinePanel({ panelRef }) {
      ...
      async function openPublicPlaylist(pl) {
        ...
        if (d && d.ok) setActivePl({ name: pl.name, creator: pl.creator, description: pl.description || d.playlist.description || '', songs: d.playlist.songs || [], source: pl.name || '酷狗歌单', public: true });
      ...
      async function openMyPlaylist(pl) {
        ...
        if (d && d.ok) setActivePl({ name: pl.name, creator: pl.creator, description: pl.description || '', songs: d.playlist.songs || [], source: pl.name || '我的歌单', listId: pl.id, mine: true });
      ...
      function openQueue() {
        ...
        if (songs.length === 0) {
          ...
          if (saved && Array.isArray(saved.queue) && saved.queue.length > 0) {
            ...
            set({ kgQueue: songs, kgSource: src || '' });
          }
        }
      ...
      async function removeFromActivePlaylist(song) {
        ...
        jsonPost('/dsh-music/kg/playlist-remove', { listId: activePl.listId, fileId: song.fileId || song.mixSongId || song.hash }).then((d) => {
          if (d && d.ok) {
            const id = String(song.hash || song.id);
      ...
      async function kgJoinAddTo(pl) {
        ...
        try {
          const d = await jsonPost('/dsh-music/kg/playlist-add', { listId: (pl && pl.id) || 0, song });
      ...
      async function kgJoinCreate() {
        ...
        openPrompt('新建歌单名称', '', async (trimmed) => {
          if (!trimmed) return;
          try {
            const d = await jsonPost('/dsh-music/kg/playlist-create', { name: trimmed });
            if (!d || !d.ok || !d.playlist) throw new Error((d && d.error) || '创建歌单失败');
            const created = d.playlist;
            if (song) {
              const add = await jsonPost('/dsh-music/kg/playlist-add', { listId: Number(created.id) || 0, song });
      ...
      const kgSongRow = (song, queue, sourceLabel, opts = {}) => {
        ...
        const vip = Number(song.payType) === 1 || Number(song.privilege) === 1;
        ...
        return React.createElement('div', { key: id, className: 'dsh-music-track-row' + (active ? ' active' : '') },
          React.createElement('button', {
            ...
            onClick: () => { if (active) togglePlay(); else startKGPlayback(song, queue, sourceLabel); },
          },
      ...
      const plCard = (pl, mine) => {
        ...
        const card = React.createElement('button', {
          ...
          React.createElement('span', { className: 'dsh-music-playlist-info' },
            ...
            React.createElement('span', { className: 'dsh-music-playlist-meta' }, (pl.creator ? pl.creator + ' · ' : '') + meta)));
      ...
      async function deleteMinePlaylist(pl) {
        ...
        openConfirm('删除歌单', '确定删除酷狗歌单「' + (pl.name || '') + '」？删除后不可恢复。', async () => {
          try {
            const d = await jsonPost('/dsh-music/kg/playlist-delete', { listId });
      ...
      if (browseTab === 'search') {
        ...
        if (searching) {
          ...
          resultContent = React.createElement('div', null,
            React.createElement('div', { className: 'dsh-music-qq-resulttabs' },
              resultTabBtn('songs', '歌曲'),
              resultTabBtn('playlists', '相关歌单')),
            resultTab === 'playlists'
              ? React.createElement('div', null, plResults.map((p) => plCard(p)), plMoreBtn)
              : React.createElement('div', null, results.map((song) => kgSongRow(song, results, '搜索结果')), songMoreBtn));
        } else if (hasSongs) {
    ...
    function QQOnlinePanel({ panelRef }) {
      ...
      async function pollLogin() {
        ...
        try {
          ...
          if (d.status === 'success') { clearPoll(); setLoggedIn(true); setUin(d.uin || ''); setNickname(d.nickname || ''); setLayer('main'); setActivePl(null); saveUi('main', '', ''); closeLogin(); refreshQQFavIds(); }
