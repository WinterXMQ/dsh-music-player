...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PRESET_CATEGORIES = ['热点', '国内', '国际', '科技', '财经', '体育'];
    ...
    function NewsPane() {
      ...
      const [models, setModels] = useState(null);        // 可选 provider/model（新闻会话模型选择器）
      ...
      const scopeSummaryText = (scope) => {
        const cats = (scope && scope.categories && scope.categories.length > 0) ? scope.categories : PRESET_CATEGORIES;
        const topics = (scope && scope.topics) ? scope.topics : [];
        return topics.length > 0 ? cats.join('/') + '+' + topics.join(',') : cats.join('/');
      };
      ...
      const saveSchedule = (next) => {
        fetch('/dsh-music/news/schedule', {
          ...
          if (d && d.ok) { setSchedule({ ...(schedule || {}), schedulePrefs: d.schedulePrefs }); setForm(JSON.parse(JSON.stringify(d.schedulePrefs))); showToast(d.changed ? '已保存' : '没有变化', true); }
      ...
      // 自动同步（v2）：优先把同步指令直接注入 agent（followup 唤醒，镜像 schedule 插件
      // 的官方投递模式），用户无需打字；agents 服务不可用（Host 返回 fallback）时回退
      // 为复制指令，由用户手动发送。
      const syncNewsSchedule = () => {
        fetch('/dsh-music/news/schedule/sync', { method: 'POST' })
          .then((r) => r.json())
          .then((d) => {
            if (d && d.ok) showToast('已自动同步：agent 正在创建定时任务', true);
            else syncNewsScheduleManual();
          })
          .catch(() => syncNewsScheduleManual());
      };
      const syncNewsScheduleManual = () => {
        const text = '同步新闻定时：请用 news_schedule {action:"get"} 读取我的面板定时偏好，按 shifts 逐条创建/更新定时任务（每班次一条，提醒内容写明收集范围与是否静默收集），多余的删除，完成后 markSynced 回写版本号。';
        copyText(text, '指令已复制，发送给 agent 即生效');
      };
      ...
      const runShiftNowManual = (shift) => {
        const scopeText = shift.scope ? scopeSummaryText(shift.scope) : scopeSummaryText(null);
      ...
      const editionRows = (s.newsEditions || []).map((e) => {
        ...
        return React.createElement('div', { key: e.id, className: 'dsh-music-track' + (playing ? ' current' : '') },
          React.createElement('div', {
            ...
            React.createElement('span', { className: 'dsh-music-track-name' },
              e.played ? '' : React.createElement('span', { style: { color: 'var(--dsw-alias-label-secondary, #8a8f98)', marginRight: 4 } }, '待播 '),
              playing ? '▶ ' : '', e.title),
            React.createElement('span', { className: 'dsh-music-track-size' }, fmtTs(e.createdAt) + (chips ? ' · ' + chips : '')),
          ),
          React.createElement('button', { className: 'dsh-music-settings-btn', title: '播放整期', onClick: () => playFrom(e.id, 0) }, '▶'),
          React.createElement('button', {
      ...
      const scheduleStatus = (() => {
        ...
        return (p.prefVersion === p.syncedVersion ? '已同步 · ' : '未同步 · ') + p.shifts.length + ' 班次';
      })();
      ...
      const statusLine = React.createElement('div', { className: 'dsh-music-subtabs', style: { marginBottom: 4 } },
        React.createElement('button', { className: 'dsh-music-subtab', title: '定时规则', style: { textAlign: 'left', flex: '1' }, onClick: () => setView({ kind: 'schedule' }) },
          '⏰ 每日定时　' + scheduleStatus + (running ? '　·　' + (running.shiftId || '手动') + ' 收集中…' : '')),
      );
      ...
      const listBody = React.createElement('div', null,
        statusLine,
        failureLine,
      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (p === null) {
          body = React.createElement('div', null, headBar('每日定时规则', { kind: 'list' }), React.createElement('div', { className: 'dsh-music-empty' }, '加载中…'));
        } else {
          const shiftRows = (p.shifts || []).map((sh, i) => {
            const scope = sh.scope; // null = 继承默认
            ...
            const setScope = (patch) => {
              const base = scope || { categories: (p.defaultScope && p.defaultScope.categories) || PRESET_CATEGORIES.slice(), topics: [] };
              setShift({ scope: { ...base, ...patch } });
            };
            const scopeRow = scope === null || scope === undefined ? null : React.createElement('div', { style: { padding: '2px 0 6px 12px' } },
              React.createElement('div', { style: { marginBottom: 4 } },
                PRESET_CATEGORIES.map((c) => React.createElement('button', {
                  key: c, className: 'dsh-music-subtab' + ((scope.categories || []).includes(c) ? ' active' : ''),
                  style: { padding: '2px 8px', marginRight: 4 },
                  onClick: () => {
                    const cur = scope.categories || [];
                    setScope({ categories: cur.includes(c) ? cur.filter((x) => x !== c) : cur.concat(c) });
                  },
                }, c)),
              ),
              React.createElement('div', null,
                (scope.topics || []).map((t) => React.createElement('button', {
                  key: t, className: 'dsh-music-subtab active', style: { padding: '2px 8px', marginRight: 4 },
                  onClick: () => setScope({ topics: scope.topics.filter((x) => x !== t) }),
                  title: '点击移除',
                }, t + ' ✕')),
                (scope.topics || []).length < LIMITS_NEWS.topicsPerShift ? React.createElement('input', {
                  value: topicDraft, placeholder: '+ 自定义主题，回车添加',
                  onChange: (e) => setTopicDraft(e.target.value),
                  onKeyDown: (e) => {
                    if (e.key === 'Enter' && topicDraft.trim() !== '') {
                      setScope({ topics: (scope.topics || []).concat(topicDraft.trim()) });
                      setTopicDraft('');
                    }
                  },
                  style: { width: 150, padding: '2px 6px' },
                }) : null,
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', style: { marginLeft: 8 }, onClick: () => setShift({ scope: null }) }, '重置为默认'),
              ),
            );
            return React.createElement('div', { key: sh.id, style: { borderBottom: '1px solid rgba(128,128,128,0.2)', padding: '4px 0' } },
              React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8 } },
                React.createElement('input', {
                  type: 'time', value: sh.time, onChange: (e) => { if (/^([01]\d|2[0-3]):[0-5]\d$/.test(e.target.value)) setShift({ time: e.target.value }); },
                  style: { padding: '2px 4px' },
                }),
                React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 4, flex: '1', cursor: 'pointer' } },
                  React.createElement('input', { type: 'checkbox', checked: sh.autoplay !== false, onChange: (e) => setShift({ autoplay: e.target.checked }) }),
                  '收集后立即播放',
                ),
                React.createElement('button', { className: 'dsh-music-settings-btn ghost', title: '立即执行该班次', onClick: () => runShiftNow(sh) }, '▶'),
                React.createElement('button', { className: 'dsh-music-icon-btn', title: '删除班次', onClick: () => setForm({ ...p, shifts: p.shifts.filter((x) => x.id !== sh.id) }) }, '🗑'),
              ),
              scopeRow,
            );
          });
          const addShift = () => {
            if ((p.shifts || []).length >= LIMITS_NEWS.shifts) return;
            const id = 's' + Math.random().toString(36).slice(2, 8);
            setForm({ ...p, shifts: (p.shifts || []).concat({ id, time: '08:00', autoplay: true, scope: null }) });
          };
          const status = (() => {
            if (!p.shifts || p.shifts.length === 0) return '未设置';
            return p.prefVersion === p.syncedVersion ? '已同步 · ' + p.shifts.length + ' 班次' : '有未同步的修改';
          })();
          // 新闻会话模型选择器（供专用「新闻简报」会话）：不选 = 跟随当前活跃会话模型。
          const modelRow = models !== null && models.length > 0 ? (() => {
            ...
            return React.createElement('div', { style: { margin: '4px 0 10px' } },
              React.createElement('div', { style: { fontWeight: 600, marginBottom: 4 } }, '新闻会话模型（专用「新闻简报」会话用）'),
              React.createElement('div', { style: { display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' } },
                React.createElement('select', {
                  ...
                  onChange: (e) => {
                    ...
                    setForm({ ...p, model: pid === '' ? null : { provider: pid, model: first ? first.id : '' } });
                  },
                ...
                curProvider ? React.createElement('select', {
                  value: sel.model || '',
                  onChange: (e) => setForm({ ...p, model: { provider: curProvider.id, model: e.target.value } }),
                  style: { padding: '2px 6px' },
                },
          ...
          body = React.createElement('div', null,
            headBar('每日定时规则', { kind: 'list' }),
            React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 6, margin: '4px 0 10px', cursor: 'pointer' } },
              React.createElement('input', { type: 'checkbox', checked: p.enabled !== false, onChange: (e) => setForm({ ...p, enabled: e.target.checked }) }),
              '启用每日定时',
            ),
            modelRow,
            React.createElement('div', { style: { fontWeight: 600, margin: '4px 0' } }, '班次（每天按以下时刻触发；点行展开收集范围）'),
            shiftRows,
            React.createElement('button', { className: 'dsh-music-settings-btn ghost', style: { margin: '6px 0 10px' }, onClick: addShift, disabled: (p.shifts || []).length >= LIMITS_NEWS.shifts }, '＋ 添加班次'),
            React.createElement('div', { style: { fontWeight: 600, margin: '4px 0' } }, '默认类别（未被班次覆盖时使用）'),
            React.createElement('div', { style: { marginBottom: 10 } },
              PRESET_CATEGORIES.map((c) => React.createElement('button', {
                key: c, className: 'dsh-music-subtab' + ((p.defaultScope && p.defaultScope.categories || []).includes(c) ? ' active' : ''),
                style: { padding: '2px 8px', marginRight: 4 },
                onClick: () => {
                  const cur = (p.defaultScope && p.defaultScope.categories) || [];
                  const next = cur.includes(c) ? cur.filter((x) => x !== c) : cur.concat(c);
                  setForm({ ...p, defaultScope: { ...(p.defaultScope || { topics: [] }), categories: next } });
                },
              }, c)),
            ),
            React.createElement('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
              React.createElement('button', { className: 'dsh-music-settings-btn', onClick: () => saveSchedule(p) }, '保存'),
              React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: syncNewsSchedule }, '同步到定时任务'),
              React.createElement('span', { className: 'dsh-music-track-size' }, '同步状态：' + status),
            ),
            React.createElement('div', { className: 'dsh-music-track-size', style: { marginTop: 8 } },
              'ⓘ 点「同步到定时任务」会自动创建/复用专用「新闻简报」会话并生成 DSH 定时任务，无需手动开会话；定时任务与每轮收集都归属该会话。'),
          );
        }
      } else {
        body = listBody;
    ...
    function PlayerPanel() {
      ...
      const listBody = React.createElement('div', { className: 'dsh-music-list-body' },
        ...
        React.createElement('div', { style: paneStyle('news') }, React.createElement(NewsPane, null)),
    ...
    function About() {
      ...
      return React.createElement('div', { className: 'dsh-music-about' },
        React.createElement('div', { className: 'dsh-music-about-card' },
          ...
          infoRow('AI 讲书', ttsTxt, s.ttsConfigured ? 'ok' : 'err'),
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-about-k { flex: none; width: 72px; color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
