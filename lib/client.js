...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PREF_BOOK_PLAYBACK = 'dsh-music-book-playback';
    ...
    function savePlayback() {
      if (store.currentId === null) { clearPref(PREF_PLAYBACK); clearPref(PREF_BOOK_PLAYBACK); return; }
      if (String(store.currentId).startsWith('book:')) {
        // Books cache their own richer state: chunk index, the cumulative
        // book-wide clock, and the in-chunk position, so a refresh can resume
        // the exact spot with a continuous time readout.
        clearPref(PREF_PLAYBACK);
        const id = currentBookId();
        if (id === null) { clearPref(PREF_BOOK_PLAYBACK); return; }
        savePref(PREF_BOOK_PLAYBACK, JSON.stringify({
          id, name: store.currentName,
    ...
    // Accent color for the spectrum bars, read from the theme-derived
    // --dsh-music-accent (see PLAYER_CSS), cached once.
    ...
    const bookUrl = (id, from) => {
      const b = bookById(id);
      return b ? b.url + '?from=' + from : null;
    };
    async function ensureBookTotal(id) {
      if (bookTotal >= 0) return bookTotal;
      const book = bookById(id); if (book === null) return -1;
      try {
        const r = await fetch(book.url + '/meta', { cache: 'no-store' });
        if (r.ok) { const m = await r.json(); if (m && typeof m.total === 'number') bookTotal = m.total; }
      } catch {}
      return bookTotal;
    }
    ...
    function playBook(id) { unlockAutoplay(); bookTotal = -1; bookBufferedFrom = -1; bookBaseTime = 0; bookRestorePos = -1; clearPref(PREF_BOOK_PLAYBACK); playBookFrom(id, 0, false); }
    ...
    function togglePlay() {
      ...
      if (audio.paused) {
        ...
        if (restoredMusicPos !== null && restoredMusicPos > 0) {
          const track = trackById(store.currentId);
    ...
    function stop() {
      ...
      audio.load();
      stopBookHelper();
      set({ currentId: null, currentName: null, playing: false, position: 0, duration: 0, pendingId: null, pendingName: null, vizState: 'ok', bookBuffering: false, bookError: '', bookBufferingSince: 0, bookBufferingSilent: false });
      clearPref(PREF_PLAYBACK);
      clearPref(PREF_BOOK_PLAYBACK);
      releaseWakeLock();
    }
    ...
    // Restore a novel's playback after a refresh: same book, same chunk, same
    // cumulative clock, and same in-chunk position — paused (tap ▶ to resume),
    // matching how music is restored. The chunk URL is loaded in the background
    // so resuming is instant (and the next chunk gets pre-warmed).
    function restoreBookPlayback() {
      const raw = loadPref(PREF_BOOK_PLAYBACK);
      if (raw === null) return;
      let saved;
      try { saved = JSON.parse(raw); } catch (e) { return; }
      if (saved === null || typeof saved !== 'object' || typeof saved.id !== 'string') return;
      const book = store.books.find((b) => b.id === saved.id);
      if (book === undefined) return; // the book is no longer in the library
      // Guard against the scan order shifting and reusing this id for a
      // different file: only restore when the saved name still matches.
      if (typeof saved.name === 'string' && saved.name !== '' && saved.name !== book.name) return;
      const from = Number.isFinite(saved.from) && saved.from >= 0 ? saved.from : 0;
      ...
      bookFromRef = from;
      bookBaseTime = base;
      const url = bookUrl(saved.id, from);
      if (url === null) return;
      ...
      set({
        currentId: 'book:' + saved.id, currentName: book.name,
      ...
      bookTotal = -1;
      void ensureBookTotal(saved.id).then((total) => {
        if (!Number.isFinite(total) || total < 0) { bookTotal = savedTotal; return; }
        bookTotal = total;
        if (from + 1 < total) preloadBook(saved.id, from + 1);
      });
    }
    ...
    function PlayerPanel() {
      ...
      const bookRows = s.books.map((b) => {
        ...
        return React.createElement('button', {
          ...
          className: 'dsh-music-track' + (active ? ' active' : ''),
          title: b.url,
          onClick: () => { if (active) togglePlay(); else playBook(b.id); },
        },
          React.createElement('span', { className: 'dsh-music-track-name' }, (playing ? '\u25b6 ' : '') + b.name),
          React.createElement('span', { className: 'dsh-music-track-size' }, b.size ? Math.round(b.size / 1024) + ' KB' : ''),
        );
      });
    ...
    function DirectorySetting() {
      ...
      const hint = isBook
        ? '\u652f\u6301 .txt \u6587\u4ef6\uff0c\u7ecf AI \u8bb2\u4e66\u64ad\u653e\uff08\u9700\u914d\u7f6e xiaomi/MiMo TTS \u6a21\u578b\uff09\u3002'
    ...
    const PLAYER_CSS = '\n' +
      ...
      ':root { --dsh-music-accent: var(--dsw-alias-brand-primary, #2f9e6e); }\n' +
