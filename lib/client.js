 ...
 * position. Play mode and volume persist across reloads via localStorage; the
 * current track + position are restored without autoplay (a tap on ▶ resumes).
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 中文对话引号对：按句切字幕时把它们视为整体，引号内出现的 。！？…\n 不作为断点，
    // 避免把「你来了吗？」一段对话拦腰截断。
    const SPLIT_QUOTE_PAIRS = { '“': '”', '「': '」', '『': '』', '"': '"' };
    const isQuoteOpenC = (c) => Object.prototype.hasOwnProperty.call(SPLIT_QUOTE_PAIRS, c);
    // 讲书字幕行最长长度（超过时再按自然停顿自适应断行），以及行内自然停顿字符。
    // 注意：SUBTITLE_MAX 是「去标点后的字数」上限——标点不计入，避免标点挤占字数。
    const SUBTITLE_MAX = 20;
    // 断行优先的自然停顿字符。除分句停顿（，、：；——…）外，也包含句末标点（。！？）：
    // splitSentences 会把引号内的句末标点跳过（只在引号收尾断句），所以超长段落在此
    // 切分时也要能落在这些句尾上，避免生硬硬切。
    ...
    // 把一句可能超长的句子按「最多 SUBTITLE_MAX 字（去标点）」自适应断成多行：优先在
    // 逗号/顿号/句号等自然停顿处换行（不再避让引号——长对话也按内部标点优雅切断，而不是
    // 触发兜底硬切），断口后紧跟的右引号会随本行收走。仅当窗口内没有任何停顿标点时才
    // 按字数边界断。标点不计字数。
    function wrapSubtitleLine(sentence) {
      if (countSubContent(sentence) <= SUBTITLE_MAX) return [sentence];
      ...
      while (start < n) {
        ...
        for (let j = start; j < n; j++) {
          if (isSubContent(sentence[j])) cnt++;
          if (cnt > SUBTITLE_MAX) { end = j; break; }
        }
        // Don't split a surrogate pair: if end lands on a low surrogate, back off one.
        if (end < n && isSubLowSurrogate(sentence.charCodeAt(end))) end--;
        // Last natural clause break inside [start, end) — 不区分是否在引号内。
        let brk = -1;
        for (let j = start; j < end; j++) {
          if (SUBTITLE_CLAUSE_BREAKS.indexOf(sentence[j]) !== -1) brk = j;
        }
        let lineEnd = brk > start ? brk + 1 : Math.max(start + 1, end);
        // 断句后把紧跟在后面的右引号一并收进本行（如 「。.””」），避免引号单独成行。
        while (lineEnd < n && SUB_QUOTE_CLOSE.has(sentence[lineEnd])) lineEnd++;
        lines.push(sentence.slice(start, lineEnd));
        start = lineEnd;
      }
      return lines;
    }
    // 按句 + 引号整体切分。断点：句末标点（。！？…\n，引号外）以及「双引号收尾处」。
    // 引号内的 。？ 不作为断点（对话保持整体），但整个对话到其右引号处算一个完整片段，
    // 即「“... ”」结束后立即断句。
    function splitSentences(text) {
      ...
      const flush = (end) => {
        ...
        if (piece !== '') out.push(...wrapSubtitleLine(piece));
        start = end;
      };
      while (i < raw.length) {
        const c = raw[i];
        if (isQuoteOpenC(c)) {
          const close = raw.indexOf(SPLIT_QUOTE_PAIRS[c], i + 1);
          if (close !== -1) {
            // 双引号结束之处断句：把「从当前 start 到右引号）的整段（含引号）」作为一个片段。
            flush(close + 1);
            i = close + 1;
            continue;
          }
    ...
    function resetLyric() {
      ...
      subtitleWeights = [];
      subtitleDur = 0;
      if (store.lyricText !== '') set({ lyricText: '' });
    }
    ...
    function updateLyric() {
      ...
      if (String(store.currentId).startsWith('book:')) {
        ...
        const idx = lyricIndexFor(audio.currentTime || 0);
        const text = idx >= 0 ? (musicLyric[idx].text || '') : '';
        if (text !== store.lyricText) set({ lyricText: text });
      }
    }
    // 拉取并解析本地曲目的 .lrc 歌词；无 .lrc / 切歌后返回皆静默。
    ...
    function loadBookSubtitle(id, from) {
      ...
      fetch('/dsh-music/book/' + id + '/text?from=' + from, { cache: 'no-store' })
        ...
        .then((d) => {
          ...
          subtitleLines = splitSentences(d.text);
    ...
    async function playBookFrom(id, from, silent) {
      ...
      set({ currentId: 'book:' + id, currentName: book.name, currentArtists: [], pendingId: null, pendingName: null, error: null, vizState: 'ok', currentSection: '', bookToc: [] });
    ...
    function NowPlayingBar() {
      ...
      return React.createElement('div', { className: 'dsh-music-bar-wrap' },
        React.createElement('div',
          ...
          hasTrack
            ? React.createElement('span', { className: 'dsh-music-bar-name', title: displayName + (artistText ? ' - ' + artistText : '') + (chapterEl ? ' - ' + s.currentSection : '') }, note, ' ', displayName, artistEl, sourceBadge, chapterEl, afterName)
          ...
          React.createElement('div', { className: 'dsh-music-bar-controls' + ((barHover || anyPopOpen) ? ' on' : '') },
            hasTrack
              ? (showHint
                  ...
                  : (isBook ? null : React.createElement('span', { className: 'dsh-music-bar-time' }, fmtTime(s.position) + ' / ' + fmtTime(s.duration))))
              : null,
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 后台静默观感）。
      '.dsh-music-bar-lyric { flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; text-align: center; color: var(--dsw-alias-label-primary, #e6e6e6); font-size: 14px; }\n' +
      ...
      // max-width:40%（与音乐「歌名 - 歌手」一致），不再单独给书名 24% 让空间。
