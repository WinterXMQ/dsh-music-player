  ...
  factory: (require) => {
    ...
    function setupLiveViz() {
      ...
      try {
        ...
        // Bar HEIGHT is governed by the dB range below (converged / less punchy). The
        // temporal smoothing keeps the bars crisp and "follow the hand": low value => the
        // analyser tracks the instantaneous FFT so the bars snap to the music (with only
        // our own rise/fall smoothing in drawViz on top).
    ...
    function drawBars(canvas, useCaps) {
      ...
      c.clearRect(0, 0, w, h);
      const gap = 2;
      const bw = 3; // fixed 3px bar width
