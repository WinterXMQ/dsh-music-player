...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PREF_KEYS = new Set([
      ...
      PREF_PLAYBACK, PREF_PLAYBACK_QQ, PREF_PLAYBACK_KG, PREF_BOOKS_PLAYBACK, PREF_QQ_FAV, PREF_QQ_HISTORY, PREF_QQ_UI,
    ...
    // 波形图垂直振幅放大系数：波形偏差围绕中线（0.5）放大该倍数，让曲线在画布高度内
    // 更"撑满"、上下起伏更明显。音乐峰值很少触及满幅（byte 0/255），默认波形只占画布
    // 高度的一小部分；放大后超出画布的上下沿会被 canvas 自然裁掉，形成顶到边的观感。
    const VIZ_WAVE_GAIN = 2.0;
    ...
    function updateLyric() {
      ...
      if (String(store.currentId).startsWith('book:')) {
        ...
        const dur = subtitleDur > 0 ? subtitleDur
          : (Number.isFinite(audio.duration) && audio.duration > 0 ? audio.duration : 0);
        if (dur <= 0) return;
        const p = Math.max(0, Math.min(1, (audio.currentTime || 0) / dur));
        ...
        if (musicWordLyric.length > 0) {
          ...
          const now2 = audio.currentTime || 0;
          ...
          set({ lyricText: text, lyricScan: scanInfo });
        } else {
    ...
    function loadBookSubtitle(id, from) {
      ...
      const book = bookById(id);
      if (book === null) return;
      fetch('/dsh-music/book/' + id + '/text?from=' + from, { cache: 'no-store' })
        .then((r) => r.json())
        .then((d) => {
    ...
    let vizFftRe = null;
    let vizFftIm = null;
    let vizScratchRe = null;     // 每帧复用的 FFT 工作缓冲（避免反复分配）
    let vizScratchIm = null;
    ...
    function setupLiveViz() {
      ...
      try {
        ...
        vizCtx = null; vizAnalyser = null; vizFreq = null; vizWave = null; vizWaveSmooth = null; vizBandSmooth = null; vizFftRe = null; vizFftIm = null; vizScratchRe = null; vizScratchIm = null; vizLive = false; vizSetupState = 2;
        return false;
      }
    }
    function closeLiveViz() {
      if (vizCtx !== null) { try { vizCtx.close(); } catch (e) {} }
      vizCtx = null; vizAnalyser = null; vizFreq = null; vizWave = null; vizWaveSmooth = null; vizBandSmooth = null; vizFftRe = null; vizFftIm = null; vizScratchRe = null; vizScratchIm = null; vizLive = false; vizSetupState = 0;
    }
    ...
    // In-place radix-2 FFT/IFFT over Float32Array re/im (equal power-of-2 length n).
    // inverse=false → forward DFT; inverse=true → unnormalized inverse DFT (the caller
    // divides by n). Standard Cooley-Tukey with bit-reversal permutation.
    ...
    // Read the analyser's time-domain samples into vizWaveSmooth (normalized 0..1,
    ...
    function analyseLiveWave() {
      ...
      // 分频段多线：对当前时域窗做一次 FFT，把频谱按 VIZ_WAVE_BANDS 分成几段，每段
      // 屏蔽其余 bin 后做逆 FFT，合成出该频段的时域波形。这样每条线反映一段频率的
      // 起伏，形成层次感。n 取 fftSize（2 的幂，满足 radix-2）。
      ...
      if (vizBandSmooth === null || vizBandSmooth.length !== VIZ_WAVE_BANDS.length * n) {
        ...
        vizFftRe = new Float32Array(n);
        vizFftIm = new Float32Array(n);
        // 复用工作缓冲，避免每帧 slice() 反复分配（60fps 下会很卡）。
        vizScratchRe = new Float32Array(n);
        vizScratchIm = new Float32Array(n);
        for (let i = 0; i < vizBandSmooth.length; i++) vizBandSmooth[i] = 0.5;
      }
      const sampleRate = (vizCtx && vizCtx.sampleRate) || 48000;
      const binHz = sampleRate / n;
      // 整窗 FFT（用 byte 中心 128 减去，去除直流偏置后再变换）。
      for (let i = 0; i < n; i++) { vizFftRe[i] = vizWave[i] - 128; vizFftIm[i] = 0; }
      fftRadix2(vizFftRe, vizFftIm, false);
      const half = n >> 1;
      // 每段：屏蔽段外 bin（连同共轭对称的负频 bin 一起），逆 FFT 得到该段时域。
      for (let b = 0; b < VIZ_WAVE_BANDS.length; b++) {
        const band = VIZ_WAVE_BANDS[b];
        const lo = Math.max(1, Math.min(half - 1, Math.floor(band.lo / binHz)));
        const hi = Math.max(lo + 1, Math.min(half - 1, Math.ceil(band.hi / binHz)));
        // 把当前窗的频谱复制到复用缓冲并清零段外 bin，保留 Hermitian 对称（含负频部分）。
        const tre = vizScratchRe; const tim = vizScratchIm;
        tre.set(vizFftRe); tim.set(vizFftIm);
        for (let k = 0; k < n; k++) {
          const idx = k <= half ? k : n - k; // 到尼奎斯特的对称索引
          if (idx < lo || idx > hi) { tre[k] = 0; tim[k] = 0; }
        }
        fftRadix2(tre, tim, true);
        const base = b * n;
        for (let i = 0; i < n; i++) {
          // IFFT 未归一化：除以 n；值范围 ±(hi-lo) 量级，压缩到 0..1 并围绕 0.5。
          const v = 0.5 + tre[i] / n / 255;
          // 对称平滑：上升/下降用相同系数，避免"峰值悬高"造成波形整体偏上。
          const s = vizBandSmooth[base + i];
          vizBandSmooth[base + i] = s + (v - s) * 0.3;
        }
      }
      // 单条回落波形（vizWaveSmooth）同样以 0.5 为中线、对称平滑。
    ...
    function drawWaveform(canvas) {
      ...
      for (let b = 0; b < bands.length; b++) {
        ...
        drawOneWave(c, w, h, n, src, bands[b].alpha, bands[b].width, color);
      }
    }
    ...
    function drawOneWave(c, w, h, n, samples, alpha, width, color) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const norm = 0.5 + (samples[i] - mean) * VIZ_WAVE_GAIN;
    ...
    function togglePanel() {
      ...
      if (opening) {
        ...
        set({ tab: isBook ? 'book' : (isQQ ? 'qq' : (isKG ? 'kg' : 'music')) });
      }
    ...
    async function playBookFrom(id, from, silent) {
      ...
      set({ currentId: 'book:' + id, currentName: book.name, currentArtists: [], pendingId: null, pendingName: null, error: null, currentSection: '', bookToc: [], currentQuality: '', bookProgress: 0 });
    ...
    function restoreLatest(list) {
      ...
      if (best === 'book') { restoreBookPlayback(); return; }
      if (best === 'qq') { restoreQQPlayback(); return; }
      if (best === 'kg') { restoreKGPlayback(); return; }
    ...
    //        └ run(跑马灯平移层；不溢出时静止)
    //            └ fx(key=seq，入场动画；上一行文本放 data-prev 供 CSS 伪元素做退场动画)
    // 关键设计：退场的「旧一句」放在 ::after 伪元素里（content: attr(data-prev)）——
    // 伪元素文本不属于 DOM textContent，读屏与测试只看到当前行，退场层零副作用。
    ...
    function BarLyric({ text, fx, playing, scan, src }) {
      ...
      // 溢出测量：run 实宽 − 可视宽。jsdom 无布局恒为 0 → 自动退化为静态居中，
      // 真浏览器由 ResizeObserver/resize 事件驱动重测。
      const [overPx, setOverPx] = useState(0);
      useEffect(() => {
        const measure = () => {
          const c = clipRef.current, r = runRef.current;
          if (!c || !r || typeof c.clientWidth !== 'number' || c.clientWidth <= 0) { setOverPx(0); return; }
          const over = Math.ceil(Math.max(0, r.scrollWidth - c.clientWidth));
          setOverPx((prev) => (Math.abs(prev - over) < 2 ? prev : over));
        };
        measure();
        let ro = null;
      ...
      return React.createElement('span',
        {
          ...
          'data-over': overPx > 0 ? '1' : undefined,
        },
    ...
    function NowPlayingBar() {
      ...
      // 取到「真实品质」时附加「 · 无损/高音质/标准」，取不到则只显示「QQ音乐」。
      let sourceBadge = null;
      if (showQuality && s.currentId !== null && String(s.currentId).startsWith('qq:')) {
        const q = (s.currentQuality || '').trim();
        sourceBadge = React.createElement('span', { className: 'dsh-music-bar-src', title: q ? ('QQ音乐（在线）· ' + q) : 'QQ音乐（在线）' },
          'QQ音乐' + (q ? ' · ' + q : ''));
      }
      // 本地音乐的「格式 · 音质」标签（如 FLAC · 无损 / MP3 · 高音质），与在线三档一致；
      // 解析不出（未知格式/不可读）则不显示。在线 QQ 用上面的 QQ音乐 徽标，互不叠加。
      let localQualityBadge = null;
      if (showQuality && s.currentId !== null && !String(s.currentId).startsWith('qq:') && !String(s.currentId).startsWith('book:') && s.currentQuality) {
      ...
      return React.createElement('div', { id: 'dsh-music-bar-wrap', className: 'dsh-music-bar-wrap' },
        React.createElement('div',
          { className: 'dsh-music-bar' + (isBook ? ' book' : '') + (barDimmed ? ' dimmed' : '') + (s.showBarBg ? '' : ' bare'), style: barStyle, onMouseEnter: showBarBtns, onMouseLeave: onBarLeave },
          hasTrack
    ...
    function startKGPlayback(song, queue, sourceLabel) {
    ...
    function KGOnlinePanel({ panelRef }) {
          ...
          const tick = () => {
            fetch('/dsh-music/kg/login/check?key=' + encodeURIComponent(d.key), { cache: 'no-store' })
              ...
              .then((c) => {
                ...
                if (c.status === 'success') {
                  setLoggedIn(true); setUserid(c.userid || ''); setLoginMsg('登录成功');
                  setQrImage(''); setQrKey('');
                  refreshMine();
      ...
      function doLogout() {
        jsonPost('/dsh-music/kg/login/logout', {}).then(() => {
          // 与 QQ 一致：退出要停掉当前酷狗曲目并清空队列与持久化（避免跨账号串号）。
          ...
          set({ kgQueue: [], kgSource: '' });
        }).catch(() => {});
      }

      // ---- 两层 UI：main / playlist ----
      const [layer, setLayer] = useState('main');
      const [activePl, setActivePl] = useState(null); // { name, songs, source, listId?, public? }
      // 子标签 + 各区数据（各自懒加载，进面板常驻不丢）
      const [browseTab, setBrowseTab] = useState('recommend'); // recommend|category|tops|search|mine
      // 搜索
      ...
      function refreshMine() {
        ...
        json('/dsh-music/kg/my-playlists').then((d) => {
          if (d && d.ok) { setMinePlays(d.playlists || []); setMineLoaded(true); }
          else setMineErr((d && d.error) || '加载失败');
        }).catch(() => setMineErr('网络错误'));
      }
      function openRecommended() {
        setRecLoading(true); setBrowseErr('');
        json('/dsh-music/kg/playlists').then((d) => {
          if (d && d.ok) setRecs(d.playlists || []); else setBrowseErr(d.error || '加载失败');
        }).catch(() => setBrowseErr('网络错误')).finally(() => setRecLoading(false));
      }
      function openCategories() {
        json('/dsh-music/kg/playlist-categories').then((d) => { if (d && d.ok) setCats(d.categories || []); }).catch(() => {});
      }
      function openCategory(cat) {
        setCurCategory(cat); setCatPlays([]); setCatPage(1); setCatHasMore(true); setBrowseErr('');
        json('/dsh-music/kg/playlists?category=' + encodeURIComponent(cat.id) + '&page=1').then((d) => {
          if (d && d.ok) { setCatPlays(d.playlists || []); setCatHasMore((d.playlists || []).length >= 20); }
        }).catch(() => {});
      }
      function moreCategory() {
        if (!curCategory) return;
        json('/dsh-music/kg/playlists?category=' + encodeURIComponent(curCategory.id) + '&page=' + (catPage + 1)).then((d) => {
          if (d && d.ok) {
            const arr = d.playlists || [];
            setCatPlays(catPlays.concat(arr)); setCatPage(catPage + 1); setCatHasMore(arr.length >= 20);
          } else setCatHasMore(false);
        }).catch(() => setCatHasMore(false));
      }
      function openTops() {
        setTopLoading(true); setBrowseErr('');
        json('/dsh-music/kg/top-lists').then((d) => {
          if (d && d.ok) setTopGroups(d.groups || []); else setBrowseErr(d.error || '加载失败');
        }).catch(() => setBrowseErr('网络错误')).finally(() => setTopLoading(false));
      }
      function openTop(top) {
        setTopLoading(true); setBrowseErr('');
        json('/dsh-music/kg/top-songs?rankId=' + encodeURIComponent(top.id) + '&offset=0&num=30').then((d) => {
          if (d && d.ok) {
            const t = d.toplist;
            setTopDetail({ id: top.id, name: top.name || t.name, cover: top.cover || t.cover, updateTime: t.updateTime || '', songs: t.songs || [] });
            setTopOffset((t.songs || []).length); setTopTotal(Number(t.total) || (t.songs || []).length);
          } else setBrowseErr(d.error || '加载失败');
        }).catch(() => setBrowseErr('网络错误')).finally(() => setTopLoading(false));
      }
      function moreTop() {
        if (!topDetail) return;
        json('/dsh-music/kg/top-songs?rankId=' + encodeURIComponent(topDetail.id) + '&offset=' + topOffset + '&num=30').then((d) => {
          if (d && d.ok) {
            const arr = d.toplist.songs || [];
            setTopDetail({ ...topDetail, songs: topDetail.songs.concat(arr) });
            setTopOffset(topOffset + arr.length);
          }
        }).catch(() => {});
      }
      function doSearch(e) {
        ...
        const kw = q.trim();
        if (kw === '') return;
        setSearching(true); setSearched(true); setQError('');
        Promise.all([
          json('/dsh-music/kg/search?w=' + encodeURIComponent(kw) + '&page=1'),
          json('/dsh-music/kg/playlist-search?w=' + encodeURIComponent(kw) + '&page=1'),
        ]).then(([sg, pg]) => {
          setResults(sg && sg.ok ? sg.results : []);
          setPlResults(pg && pg.ok ? pg.playlists : []);
          if ((!sg || !sg.ok) && (!pg || !pg.ok)) setQError((sg && sg.error) || '搜索失败');
        }).catch(() => setQError('网络错误')).finally(() => setSearching(false));
      }
      function openPublicPlaylist(pl) {
        json('/dsh-music/kg/playlist/' + encodeURIComponent(pl.id)).then((d) => {
          if (d && d.ok) {
            setActivePl({ name: pl.name, creator: pl.creator, songs: d.playlist.songs || [], source: pl.name || '酷狗歌单' });
            setLayer('playlist');
          } else showToast((d && d.error) || '歌单详情加载失败', false);
        }).catch(() => showToast('网络错误', false));
      }
      function openMyPlaylist(pl) {
        json('/dsh-music/kg/my-playlist/' + encodeURIComponent(pl.id)).then((d) => {
          if (d && d.ok) {
            setActivePl({ name: pl.name, songs: d.playlist.songs || [], source: pl.name || '我的歌单', listId: pl.id, mine: true });
            setLayer('playlist');
          } else showToast((d && d.error) || '歌单内容加载失败', false);
        }).catch(() => showToast('网络错误', false));
      }
      function removeFromMyPlaylist(song) {
        if (!activePl || !activePl.listId) return;
        jsonPost('/dsh-music/kg/playlist-remove', { listId: activePl.listId, fileId: song.fileId }).then((d) => {
          if (d && d.ok) {
            const songs = activePl.songs.filter((x) => x.hash !== song.hash);
            setActivePl({ ...activePl, songs });
            showToast('已从歌单移除', true);
          } else showToast((d && d.error) || '移除失败', false);
        }).catch(() => showToast('网络错误', false));
      }
      function playAll(songs, label) {
        if (Array.isArray(songs) && songs.length > 0) startKGPlayback(songs[0], songs, label);
      }

      // ---- 渲染辅助 ----
      const fmtCount2 = (n) => { const v = Number(n) || 0; if (v >= 1e8) return (v / 1e8).toFixed(1).replace(/\.0$/, '') + '亿'; if (v >= 1e4) return (v / 1e4).toFixed(1).replace(/\.0$/, '') + '万'; return String(v); };
      const kgSongRow = (song, queue, sourceLabel, opts = {}) => {
        ...
        return React.createElement('div', { key: id, className: 'dsh-music-track-row' + (active ? ' active' : '') },
          React.createElement('button', {
            ...
            onClick: () => startKGPlayback(song, queue, sourceLabel),
          },
            React.createElement('span', { className: 'dsh-music-track-title' }, song.title || '(未知曲名)'),
            React.createElement('span', { className: 'dsh-music-track-size' },
              artists ? artists + (song.album ? ' · ' + song.album : '') : (song.album || '')),
          ),
          opts.mine && song.fileId
            ? React.createElement('button', { className: 'dsh-music-icon-btn', title: '从歌单移除', onClick: () => removeFromMyPlaylist(song) }, '✕')
            : null,
        );
      };
      const plCard = (pl) => React.createElement('button', {
        ...
        React.createElement('img', {
          className: 'dsh-music-playlist-cover', src: pl.cover || '', alt: '', loading: 'lazy',
        ...
        React.createElement('span', { className: 'dsh-music-playlist-info' },
          ...
          React.createElement('span', { className: 'dsh-music-playlist-meta' },
            (pl.creator ? pl.creator + ' · ' : '') + (pl.trackCount > 0 ? pl.trackCount + ' 首 · 播放 ' + fmtCount2(pl.playCount) : '播放 ' + fmtCount2(pl.playCount)))),
      );
      const subBtn = (key, label) => React.createElement('button', {
        className: 'dsh-music-qq-viewtab' + (browseTab === key ? ' active' : ''),
        onClick: () => {
          setBrowseTab(key);
          if (key === 'recommend' && recs.length === 0) openRecommended();
          if (key === 'category' && cats.length === 0) openCategories();
          if (key === 'tops' && topGroups.length === 0) openTops();
          if (key === 'mine') refreshMine();
        },
      }, label);
      ...
      if (!loggedIn) {
        return React.createElement('div', { className: 'dsh-music-qq' },
          React.createElement('div', { className: 'dsh-music-qq-login' },
            React.createElement('div', { className: 'dsh-music-qq-login-center' },
              qrImage
                ? React.createElement('img', { src: qrImage, alt: '酷狗登录二维码', width: 180, height: 180 })
                : null,
              React.createElement('div', { className: 'dsh-music-qq-login-status' },
                loginMsg || '酷狗音乐需扫码登录后才能试听'),
              !qrImage && React.createElement('button', { className: 'dsh-music-qq-login-btn', disabled: loginBusy, onClick: startLogin }, '生成酷狗登录二维码'),
              qrImage && loginMsg.indexOf('过期') !== -1 && React.createElement('button', { className: 'dsh-music-qq-login-btn', onClick: startLogin }, '重新获取二维码'),
              React.createElement('div', { className: 'dsh-music-qq-login-warn' },
                React.createElement('div', { className: 'dsh-music-qq-login-warn-item' },
                  React.createElement('span', { className: 'dsh-music-qq-login-warn-num' }, '·'),
                  React.createElement('p', { className: 'dsh-music-qq-login-warn-p' }, '登录仅存于本机 Host（~/.dsh 下 0600 文件），用于换取试听直链；VIP 曲目仅会员账号可听高音质。')),
                React.createElement('div', { className: 'dsh-music-qq-login-warn-item' },
                  React.createElement('span', { className: 'dsh-music-qq-login-warn-num' }, '·'),
                  React.createElement('p', { className: 'dsh-music-qq-login-warn-p' }, '非官方接口仅供个人学习试听，勿用于商业用途。')),
              ),
            )));
      }
      // playlist 层
      if (layer === 'playlist' && activePl) {
        return React.createElement('div', { className: 'dsh-music-qq' },
          React.createElement('div', { className: 'dsh-music-qq-head' },
            React.createElement('button', { className: 'dsh-music-icon-btn', onClick: () => setLayer('main') }, '← 返回'),
            React.createElement('span', null, activePl.name || activePl.source || '播放列表'),
            activePl.songs.length > 0
              ? React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: () => playAll(activePl.songs, activePl.source) }, '▶ 全部播放')
              : null),
          React.createElement('div', { className: 'dsh-music-qq-body' },
            activePl.songs.length === 0
              ? React.createElement('div', { className: 'dsh-music-empty' }, '暂无歌曲')
              : activePl.songs.map((song) => kgSongRow(song, activePl.songs, activePl.source, { mine: !!activePl.mine })),
          ));
      }
      // main 层：命令式组装各子标签的渲染片段（避免深层嵌套表达式难以维护）
      const bodyKids = [];
      const pushCards = (list) => { for (const pl of list) bodyKids.push(plCard(pl)); };
      if (browseTab === 'recommend') {
        if (recLoading) bodyKids.push(React.createElement('div', { key: 'l', className: 'dsh-music-empty' }, '加载中…'));
        else if (recs.length === 0) bodyKids.push(React.createElement('div', { key: 'e', className: 'dsh-music-empty' }, browseErr || '暂无数据'));
        else pushCards(recs);
      } else if (browseTab === 'category') {
        const catKids = [];
        for (const c of cats.slice(0, catOpen ? cats.length : 18)) {
          catKids.push(React.createElement('button', {
            key: c.group + ':' + c.id, className: 'dsh-music-qq-cat-toggle', onClick: () => openCategory(c),
          }, c.name));
        }
        if (cats.length > 18) catKids.push(React.createElement('button', {
          key: 'x', className: 'dsh-music-qq-cat-toggle', onClick: () => setCatOpen(!catOpen),
        }, catOpen ? '收起' : '展开全部'));
        bodyKids.push(React.createElement('div', { key: 'c', style: { padding: '6px 12px', flexWrap: 'wrap', gap: 6, display: 'flex' } }, catKids));
        if (curCategory) bodyKids.push(React.createElement('div', { key: 'h', style: { padding: '0 12px', fontSize: 13, fontWeight: 600 } }, '「' + curCategory.name + '」的歌单'));
        for (const pl of catPlays) bodyKids.push(plCard(pl));
        if (curCategory && catHasMore && catPlays.length > 0) {
          bodyKids.push(React.createElement('div', { key: 'm', className: 'dsh-music-qq-loadmore' },
            React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: moreCategory }, '加载更多')));
        }
      } else if (browseTab === 'tops') {
        if (topDetail) {
          const headKids = [
            React.createElement('button', { key: 'b', className: 'dsh-music-icon-btn', onClick: () => setTopDetail(null) }, '← 榜单'),
            React.createElement('b', { key: 'n' }, topDetail.name),
          ];
          if (topDetail.updateTime) headKids.push(React.createElement('span', { key: 'u', className: 'dsh-music-track-size' }, ' 更新：' + topDetail.updateTime));
          if (topDetail.songs.length > 0) headKids.push(React.createElement('button', { key: 'p', className: 'dsh-music-qq-loadmore-btn', onClick: () => playAll(topDetail.songs, topDetail.name) }, '▶ 播放全部'));
          bodyKids.push(React.createElement.apply(null, ['div', { key: 'hd', className: 'dsh-music-qq-topdetail-head' }].concat(headKids)));
          let i = 0;
          for (const song of topDetail.songs) { i++; bodyKids.push(kgSongRow(song, topDetail.songs, topDetail.name)); }
          if (topDetail.songs.length < topTotal) {
            bodyKids.push(React.createElement('div', { key: 'm', className: 'dsh-music-qq-loadmore' },
              React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', onClick: moreTop },
                '加载更多（已有 ' + topDetail.songs.length + '/' + topTotal + '）')));
          }
        } else if (topLoading) {
          bodyKids.push(React.createElement('div', { className: 'dsh-music-empty' }, '加载中…'));
        } else if (topGroups.length === 0) {
          bodyKids.push(React.createElement('div', { className: 'dsh-music-empty' }, browseErr || '暂无榜单'));
        } else {
          for (const t of (topGroups[0].toplists || [])) {
            bodyKids.push(React.createElement('button', {
              key: t.id, className: 'dsh-music-qq-topitem', onClick: () => openTop(t),
            },
              t.cover ? React.createElement('img', { src: t.cover, alt: '', width: 48, height: 48 }) : null,
              React.createElement('span', null, t.name),
              t.updateTime ? React.createElement('small', { className: 'dsh-music-track-size' }, ' ' + t.updateTime) : null));
          }
        }
      } else if (browseTab === 'search') {
        bodyKids.push(React.createElement('form', { key: 'f', className: 'dsh-music-qq-search', onSubmit: doSearch },
          React.createElement('input', {
            className: 'dsh-music-qq-input', placeholder: '搜歌曲 / 歌单（酷狗）', value: q,
            onChange: (ev) => setQ(ev.target.value),
          }),
          React.createElement('button', { className: 'dsh-music-qq-loadmore-btn', type: 'submit', disabled: searching }, searching ? '搜索中…' : '搜索')));
        if (qError) bodyKids.push(React.createElement('div', { key: 'e', className: 'dsh-music-empty' }, qError));
        if (searched && !searching && results.length + plResults.length === 0 && qError === '') {
          bodyKids.push(React.createElement('div', { key: 'n', className: 'dsh-music-empty' }, '没有找到「' + q.trim() + '」的相关内容'));
        }
        if (results.length > 0 || plResults.length > 0) {
          bodyKids.push(React.createElement('div', { key: 'st', className: 'dsh-music-qq-viewtabs' },
            React.createElement('button', { className: 'dsh-music-qq-viewtab' + (resultTab === 'songs' ? ' active' : ''), onClick: () => setResultTab('songs') }, '歌曲 ' + results.length),
            React.createElement('button', { className: 'dsh-music-qq-viewtab' + (resultTab === 'playlists' ? ' active' : ''), onClick: () => setResultTab('playlists') }, '歌单 ' + plResults.length)));
          if (resultTab === 'songs') { for (const song of results) bodyKids.push(kgSongRow(song, results, '搜索：' + q.trim())); }
          else pushCards(plResults);
        }
      } else if (browseTab === 'mine') {
        if (mineErr) {
          bodyKids.push(React.createElement('div', { key: 'me', className: 'dsh-music-empty', style: { color: '#e5484d' } },
            mineErr + '（点其他页签再切回可重试）'));
        } else if (mineLoaded && minePlays.length === 0) {
          bodyKids.push(React.createElement('div', { className: 'dsh-music-empty' }, '还没有我的歌单（创建能力后续版本开放）'));
        }
        for (const pl of minePlays) {
          bodyKids.push(React.createElement('button', {
            key: String(pl.id), className: 'dsh-music-playlist-card', title: pl.description || pl.name,
            onClick: () => openMyPlaylist(pl),
          },
            React.createElement('img', {
              className: 'dsh-music-playlist-cover', src: pl.cover || '', alt: '', loading: 'lazy',
              onError: (e) => { e.currentTarget.style.display = 'none'; },
            ...
            React.createElement('span', { className: 'dsh-music-playlist-info' },
              ...
              React.createElement('span', { className: 'dsh-music-playlist-meta' }, (pl.trackCount || 0) + ' 首')),
          ));
        }
      }
      return React.createElement('div', { className: 'dsh-music-qq' },
        React.createElement('div', { className: 'dsh-music-qq-head' },
          React.createElement('div', { className: 'dsh-music-qq-toolbar' },
            React.createElement('div', { className: 'dsh-music-qq-viewtabs' },
              subBtn('recommend', '推荐歌单'), subBtn('category', '分类歌单'), subBtn('tops', '排行榜'),
              subBtn('search', '搜索'), subBtn('mine', '我的歌单')),
            React.createElement('button', { className: 'dsh-music-icon-btn', title: '退出登录 (' + userid + ')', onClick: doLogout }, '退出'))),
        React.createElement('div', { className: 'dsh-music-qq-body' }, bodyKids));
    }
      ...
      function openQueue() {
        ...
        if (songs.length === 0) {
          ...
          if (saved && Array.isArray(saved.queue) && saved.queue.length > 0) {
            ...
            // 加载进内存，便于「下一首」等后续操作直接使用。
            set({ qqQueue: songs, qqSource: src || '' });
          }
        }
      ...
      return React.createElement('div', { className: 'dsh-music-panel', ref: panelRef, style: rootStyle },
        ...
        React.createElement('div', { className: 'dsh-music-panel-body' },
          ...
          React.createElement('div', { className: 'dsh-music-tabs' },
            tabBtn('music', '本地音乐'), tabBtn('qq', 'QQ音乐'), tabBtn('kg', '酷狗音乐'), tabBtn('book', 'AI讲书'), tabBtn('config', '系统配置')),
          React.createElement('div', { className: 'dsh-music-panel-content' },
            (s.tab === 'qq' || s.tab === 'kg' || s.tab === 'config') ? null : React.createElement(DirectorySetting, { panelRef }),
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 上一行经 data-prev 交给 ::after 伪元素做退场动画——伪元素不进 textContent)。
      ...
      // — 退场动画（伪元素承载上一行文本）：与入场同时进行，形成新旧句叠映的过渡。
      '.dsh-music-bar-lyric-fx[data-prev]::after { content: attr(data-prev); position: absolute; left: 0; right: 0; top: 50%; transform: translateY(-50%); pointer-events: none; animation-fill-mode: forwards; }\n' +
      ".dsh-music-bar-lyric-fx[data-fx='slide'][data-prev]::after { animation: dsh-lyric-slide-out 0.26s ease forwards; }\n" +
      ".dsh-music-bar-lyric-fx[data-fx='blur'][data-prev]::after { animation: dsh-lyric-blur-out 0.38s ease forwards; }\n" +
      ...
      "}\n" +
      // karaoke 的退场旧句必须自带实色（元素本体 color 已被 clip:text 置透明）。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke'][data-prev]::after { color: var(--dsw-alias-label-primary, #e6e6e6); -webkit-background-clip: border-box; background-clip: border-box; animation: dsh-lyric-fade-out 0.25s ease forwards; }\n" +
      '@keyframes dsh-lyric-slide-in { from { opacity: 0; transform: translateY(9px); } to { opacity: 1; transform: translateY(0); } }\n' +
      '@keyframes dsh-lyric-slide-out { to { opacity: 0; transform: translateY(-7px); } }\n' +
      '@keyframes dsh-lyric-fade-out { to { opacity: 0; } }\n' +
      ...
      '@keyframes dsh-lyric-blur-out { to { opacity: 0; filter: blur(4px); } }\n' +
      // 减动效：全部歌词动画停用；溢出行左对齐起步（跑马灯不滚动时至少能读到行首）。
      '@media (prefers-reduced-motion: reduce) { .dsh-music-bar-lyric-run.mq, .dsh-music-bar-lyric-fx { animation: none !important; } .dsh-music-bar-lyric-fx[data-audioclock] { transition: none !important; } .dsh-music-bar-lyric-run.mq { text-align: left; } .dsh-music-bar-lyric-fx[data-prev]::after { content: none; } }\n' +
