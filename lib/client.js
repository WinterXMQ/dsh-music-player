...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Restore the playback-panel position ({x,y}) previously saved by dragging, if any.
    ...
    function PlayerPanel() {
      ...
      // Draggable panel position ({x, y} left/top once dragged; null = default right/bottom)
      const [pos, setPos] = useState(loadPanelPos);
      const dragRef = useRef(null);

      const style = pos === null ? null : { left: pos.x, top: pos.y };
      ...
      const onHeadDown = (e) => {
        ...
        dragRef.current = {
          ...
          originX: pos !== null ? pos.x : rect.left,
          originY: pos !== null ? pos.y : rect.top,
        };
        e.currentTarget.setPointerCapture(e.pointerId);
      };
      const onHeadMove = (e) => {
        ...
        setPos({ x, y });
        savePref(PREF_PANEL_POS, JSON.stringify({ x, y }));
      };
      const onHeadUp = (e) => {
