...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- persisted prefs (ALL Host-backed; browser storage is not used) ----
    // Every pref below lives in the Host's music-player-prefs.json, served via
    ...
    // (keyed by origin) was therefore unreliable and is no longer used at all.
    ...
    function loadPref(k) {
      if (PREF_KEYS.has(k) && serverPrefs !== null && Object.prototype.hasOwnProperty.call(serverPrefs, k)) {
    ...
    function NowPlayingBar() {
      ...
      // 播放进度细线：有内容且已获取时长时，按 position/duration 计算填充百分比。
      ...
      return React.createElement('div', { className: 'dsh-music-bar-wrap' },
        React.createElement('div',
          ...
          // 仅音乐模式显示（AI 讲书是累计时钟、按章节推进，不做单条进度线）。
