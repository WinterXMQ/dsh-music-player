...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // slide=上滑淡入(网易云桌面词风)；fade=交叉淡化；blur=模糊浮入(Apple Music 风)；
    // karaoke=卡拉OK扫色(KTV 风格，整行匀速点亮——无逐字时间轴时的近似)。
    // 跑马灯/边缘渐隐是内置行为，不作为配置项（超宽滚动、两端渐隐恒开）。
    const LYRIC_FX_VALUES = ['none', 'slide', 'fade', 'blur', 'karaoke'];
    ...
    const store = {
      ...
      lyricScan: null,
      // 当前行的词级时间轴 [{text,s,d}]（s/d = 行内相对秒，来自 QRC 逐字流）。
      // 仅 karaoke 动效且有词级数据时启用逐字点亮渲染；null = 整行扫色。
      lyricWords: null,
    ...
    let musicWordLyric = []; // QRC 逐字歌词的精确行窗口 [{t,end,text}]（秒）。非空时优先生效
    ...
    function resetLyric() {
      ...
      subtitleWeights = [];
      subtitleDur = 0;
      if (store.lyricText !== '' || store.lyricScan !== null || store.lyricWords !== null || store.lyricSource !== '') {
        set({ lyricText: '', lyricScan: null, lyricWords: null, lyricSource: '' });
      }
    }
    ...
    function updateLyric() {
      ...
      if (String(store.currentId).startsWith('book:')) {
        ...
        set({
          ...
          lyricWords: null, // 讲书无逐字数据
        });
      } else {
        if (musicWordLyric.length > 0) {
          // —— QRC 精确行窗口：每行自带真实的结束时刻（t..end），扫色速度与演唱一致，
          // 长间奏/末行不再被摊平。定位用二分（行按 t 升序，来自 Host 解析）。
          ...
          if (text === store.lyricText) {
            // 行内校准（仅词级模式）：整行扫色的 CSS 动画不能中途改 delay（会重启闪
            // 一帧），但逐字点亮的类名由 initE/rAF 驱动——这里随 timeupdate 刷新
            // scan.elapsed，行内 seek 后下一帧就能对上真实进度；rAF 循环平时自行兜底。
            const hasW = !!(wline && Array.isArray(wline.words) && wline.words.length > 0);
            const sc = store.lyricScan;
            if (hasW && sc && Number.isFinite(sc.baseT)
              && Math.abs(Math.round((now2 - Number(wline.t)) * 1000) - sc.elapsed) > 150) {
              const durMs = Math.max(300, (Number(wline.end) - Number(wline.t)) * 1000);
              const si = scanMs(durMs, (now2 - Number(wline.t)) * 1000);
              si.baseT = Number(wline.t);
              set({ lyricScan: si });
            }
            return;
          }
          ...
          if (wline) {
            const durMs = Math.max(300, (Number(wline.end) - Number(wline.t)) * 1000);
            scanInfo = scanMs(durMs, (now2 - Number(wline.t)) * 1000);
            // baseT = 行绝对起点秒：BarLyric 的 rAF 循环用它把 audio.currentTime
            // 换算成行内已过时间（逐字点亮的实时驱动锚点）。
            if (scanInfo) scanInfo.baseT = Number(wline.t);
          }
          // 词级时间轴跟随行下发（行内相对秒）；无词级行 → null 走整行扫色。
          const words = (wline && Array.isArray(wline.words) && wline.words.length > 0)
            ? wline.words
            : null;
          set({ lyricText: text, lyricScan: scanInfo, lyricWords: words });
        } else {
          ...
          let scanInfo = null;
          if (idx >= 0) {
            // 行时长 = 下一行 t − 本行 t；末行用音频总长兜底（拿不到再退 8s 假定值，
            // 只影响扫色速度观感，不影响定位）。elapsed = 当前位置 − 行起点，钳到 ≥0。
            ...
            scanInfo = scanMs((nextT - curT) * 1000, (now - curT) * 1000);
          }
          set({ lyricText: text, lyricScan: scanInfo, lyricWords: null });
        }
      }
    }
    ...
    function loadOnlineLyric(id, track) {
      ...
      fetch('/dsh-music/lyric/online?' + params.toString(), { cache: 'no-store' })
        ...
        .then((d) => {
          if (!d || !d.ok) return;
          if (store.currentId !== id) return; // 取词期间已切歌
          // QRC 逐字歌词（精确行窗口）优先；无则回落普通 LRC。
          if (Array.isArray(d.wordLines) && d.wordLines.length > 0) {
            musicWordLyric = d.wordLines;
            const wc = d.wordLines.reduce((a, l) => a + ((l.words && l.words.length) || 0), 0);
            noteLyricSource(d.source || 'qq-qrc', d.wordLines.length + ' 行 / ' + wc + ' 词（逐字时间轴）');
            updateLyric();
            return;
          }
    ...
    // 在线 QQ 歌词：Host 按 songmid 取词（匿名，QRC 逐字优先 → 整行 LRC 兜底），
    // 逐句翻译合并进同一行（原文 ／ 翻译）。
      ...
      try {
        ...
        xhr.onload = () => {
          ...
          ac.decodeAudioData(xhr.response).then((buf) => {
            ...
            const spec = computeSpectrum(ch, buf.sampleRate);
    ...
    function computeSpectrum(ch, sampleRate) {
      ...
      const bands = new Float32Array(n * VIZ_BARS);
      const maxF = Math.min(sampleRate / 2, 18000);
      const ratio = maxF / 40;
      const edges = new Float64Array(VIZ_BARS + 1);
      for (let b = 0; b <= VIZ_BARS; b++) edges[b] = 40 * Math.pow(ratio, b / VIZ_BARS);
      ...
      let gmax = 0;
      for (let i = 0; i < n; i++) {
        const center = Math.floor((i + 0.5) * dt * sampleRate);
        for (let j = 0; j < FFT_SIZE; j++) {
          const s = center - half + j;
          re[j] = (s >= 0 && s < ch.length) ? ch[s] * win[j] : 0;
          im[j] = 0;
        }
        fftInPlace(re, im);
        const off = i * VIZ_BARS;
        for (let b = 0; b < VIZ_BARS; b++) {
          const b0 = Math.floor(edges[b] / binHz);
          const b1 = Math.min(half, Math.max(b0 + 1, Math.ceil(edges[b + 1] / binHz)));
          let m = 0;
          for (let k = b0; k < b1; k++) {
    ...
    // 五种 fx：none=硬切(无 key 不重挂)；slide=上滑淡入；fade=交叉淡化；
    // blur=模糊浮入(Apple Music 风)；karaoke=扫色。karaoke 有词级时间轴（QRC）时
    // 升级为逐字点亮：每个词一个 span，初始类按 scan.elapsed 静态判定，浏览器里再由
    // rAF 循环实时驱动 done/act 类（读 audio.currentTime，天然免疫行内 seek 漂移，
    // 且暂停时停帧保持当前高亮）。无词级行回落整行渐变扫色。
    // 跑马灯与边缘渐隐为内置行为（恒开，不是配置项）：超宽行自动滚动，两端渐隐。
    function BarLyric({ text, fx, playing, scan, words, src }) {
      const clipRef = useRef(null);
      const runRef = useRef(null);
      const wordRefs = useRef([]);       // 词 span 引用（逐字模式）
      const useWords = fx === 'karaoke' && Array.isArray(words) && words.length > 0;
      const initE = Number(scan && scan.elapsed) || 0;   // 渲染期初值：行内已过毫秒
      ...
      // 逐字点亮驱动：rAF 每帧按 audio.currentTime 重算各词 done/act 类。
      // effect 只在行/播放态变化时重建；词数组身份随换行变化，天然重置。
      useEffect(() => {
        if (!useWords || !Array.isArray(words)) return;
        let alive = true;
        let rafId = 0;
        const baseT = (typeof scan === 'object' && scan !== null && Number.isFinite(scan.baseT))
          ? scan.baseT
          : null;
        const step = () => {
          if (!alive) return;
          try {
            const nowSec = (audio && typeof audio.currentTime === 'number') ? audio.currentTime : null;
            if (nowSec !== null && baseT !== null) {
              const e = (nowSec - baseT) * 1000;
              for (let i = 0; i < words.length; i++) {
                const node = wordRefs.current[i];
                if (!node) continue;
                const ws = (Number(words[i].s) || 0) * 1000;
                const wd = Math.max(1, (Number(words[i].d) || 0) * 1000);
                node.classList.toggle('done', e >= ws + wd);
                node.classList.toggle('act', !!playing && e >= ws && e < ws + wd);
              }
            }
          } catch { /* 帧级更新失败不影响渲染 */ }
      ...
      const runStyle = mqOn
        ...
        : undefined;
      const karStyle = (fx === 'karaoke' && !useWords && scan && Number(scan.dur) > 0)
        ? { '--kar-dur': scan.dur + 'ms', '--kar-delay': (-Number(scan.elapsed) || 0) + 'ms' }
        : undefined;
      // 逐字模式不用整行渐变扫色变量；时间锚点 scan.baseT 由 rAF 循环读取 audio 换算。
      const fxStyle = useWords ? undefined : karStyle;
      // 词的静态初始分类（供 jsdom/首帧）：done=已唱完、act=正在唱；浏览器里 rAF 接管。
      const wordCls = (w) => {
        const ws = (Number(w.s) || 0) * 1000;
        const wd = Math.max(1, (Number(w.d) || 0) * 1000);
        if (initE >= ws + wd) return 'dsh-music-word done';
        if (initE >= ws) return 'dsh-music-word act';
        return 'dsh-music-word';
      };
      const fxChildren = useWords
      ...
      return React.createElement('span',
        ...
        text === '' ? null : React.createElement('span', {
          ...
          'data-fx': fx,
          'data-wordmode': useWords ? '1' : undefined,
    ...
    function NowPlayingBar() {
      ...
      if (s.currentId !== null && !String(s.currentId).startsWith('qq:') && !String(s.currentId).startsWith('book:') && s.currentQuality) {
        ...
        React.createElement('div',
          ...
          s.lyricText && barDimmed && hasTrack && s.showLyric
            ? React.createElement(BarLyric, {
                ...
                playing: s.playing, scan: s.lyricScan, words: s.lyricWords,
                src: s.lyricSource,
              })
            : null,
    ...
    // 用轻量 HEAD 探测「真实品质」：不要在频谱信封（loadEnvelope）下载完整音频后才读
    ...
    function SystemSetting() {
      ...
      // 歌词动效：分段选择器（单选按钮组）。参照主流播放器的五种换行风格：
      // none=无动效（硬切）｜slide=上滑淡入（网易云桌面词）｜fade=交叉淡化
    ...
    const PLAYER_CSS = '\n' +
      ...
      ".dsh-music-bar-lyric-fx[data-fx='fade'] { animation: dsh-lyric-fade-in 0.34s ease backwards; }\n" +
      ".dsh-music-bar-lyric-fx[data-fx='blur'] { animation: dsh-lyric-blur-in 0.42s ease-out backwards; }\n" +
      // 首次挂载（没有上一行）：推迟出现，避开按钮组收起过程（见上方注释）。
      ".dsh-music-bar-lyric-fx:not([data-prev])[data-fx='slide'], .dsh-music-bar-lyric-fx:not([data-prev])[data-fx='fade'], .dsh-music-bar-lyric-fx:not([data-prev])[data-fx='blur'] { animation-delay: 0.28s; }\n" +
      // — 退场动画（伪元素承载上一行文本）：与入场同时进行 = 视觉交叉淡化。
      ...
      ".dsh-music-bar-lyric-fx[data-fx='fade'][data-prev]::after { animation: dsh-lyric-fade-out 0.3s ease forwards; }\n" +
      ...
      // —— 整行扫色（无词级时间轴时）：:not([data-wordmode]) 让逐字模式完全绕开渐变。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke']:not([data-wordmode]) { background-image: linear-gradient(90deg, var(--dsh-music-accent, #2f9e6e) 0%, var(--dsh-music-accent, #2f9e6e) 41%, color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 55%, #fff) 42%, color-mix(in srgb, var(--dsw-alias-label-primary, #e6e6e6) 40%, transparent) 44%, color-mix(in srgb, var(--dsw-alias-label-primary, #e6e6e6) 40%, transparent) 100%); background-size: 250% 100%; background-repeat: no-repeat; -webkit-background-clip: text; background-clip: text; color: transparent; animation: dsh-kar-sweep var(--kar-dur, 6s) linear var(--kar-delay, 0s) backwards, dsh-kar-in 0.22s ease backwards; }\n" +
      // —— 逐字点亮模式（QRC 词级时间轴）：Apple Music 风三态——未唱暗场 / 已唱实色 /
      // 正唱高光加微光晕；颜色由类切换 + 短过渡驱动，rAF 实时更新类。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke'][data-wordmode] { animation: dsh-kar-in 0.22s ease backwards; }\n" +
      '.dsh-music-word { transition: color 0.12s ease, text-shadow 0.12s ease; }\n' +
      '.dsh-music-word { color: color-mix(in srgb, var(--dsw-alias-label-primary, #e6e6e6) 40%, transparent); }\n' +
      '.dsh-music-word.done { color: var(--dsh-music-accent, #2f9e6e); }\n' +
      '.dsh-music-word.act { color: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 65%, #fff); text-shadow: 0 0 9px color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 60%, transparent); }\n' +
      ...
      ".dsh-music-bar-lyric-fx[data-fx='karaoke']:not([data-wordmode]) { background-color: transparent; }\n" +
      "@supports not (background: color-mix(in srgb, red 50%, blue)) {\n" +
      "  .dsh-music-bar-lyric-fx[data-fx='karaoke']:not([data-wordmode]) { color: var(--dsw-alias-label-primary, #e6e6e6); }\n" +
      "  .dsh-music-word { color: var(--dsw-alias-label-primary, #e6e6e6); }\n" +
      "  .dsh-music-word.done, .dsh-music-word.act { color: var(--dsh-music-accent, #2f9e6e); }\n" +
      "}\n" +
      // karaoke 的退场旧句必须自带实色（元素本体 color 已被 clip:text 置透明）。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke']:not([data-wordmode])[data-prev]::after { color: var(--dsw-alias-label-primary, #e6e6e6); -webkit-background-clip: border-box; background-clip: border-box; animation: dsh-lyric-fade-out 0.25s ease forwards; }\n" +
      // 逐字模式的退场：基础 [data-prev]::after 规则已给出 content/定位，这里只补
      // 实色（词 span 颜色自足，无需 clip:text 反制）+ 淡出动画。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke'][data-wordmode][data-prev]::after { color: var(--dsw-alias-label-primary, #e6e6e6); animation: dsh-lyric-fade-out 0.25s ease forwards; }\n" +
      ...
      '@keyframes dsh-lyric-fade-in { from { opacity: 0; } to { opacity: 1; } }\n' +
      ...
      '@media (prefers-reduced-motion: reduce) { .dsh-music-bar-lyric-run.mq, .dsh-music-bar-lyric-fx { animation: none !important; } .dsh-music-word { transition: none !important; } .dsh-music-bar-lyric-run.mq { text-align: left; } .dsh-music-bar-lyric-fx[data-prev]::after { content: none; } }\n' +
