...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function startPlay(id) {
      ...
      audio.src = track.url;
      audio.load();
      set({ currentId: id, currentName: track.name, currentArtists: track.artists || store.currentArtists || [], pendingId: null, pendingName: null, error: null, tocOpen: false, currentSection: '', qqFaved: false });
      syncShufflePos();
    ...
    function PlayerPanel() {
      ...
      const listBody = s.tab === 'music'
        ? musicBody
        : s.tab === 'qq'
          ? React.createElement(QQOnlinePanel, null)
          : (s.books.length > 0
            ? bookRows
            : (s.ttsConfigured
