    ...
    function NewsPane() {
      ...
      if (view.kind === 'detail' || view.kind === 'read') {
        ...
        if (p === null) {
          ...
          // 新闻会话模型选择器（每次执行新建的执行会话用）：不选 = 跟随当前活跃会话模型。
          const modelRow = models !== null && models.length > 0 ? (() => {
            ...
            return React.createElement('div', { style: { margin: '4px 0 10px' } },
              React.createElement('div', { style: { fontWeight: 600, marginBottom: 4 } }, '新闻会话模型（每次执行新建的执行会话用）'),
          ...
          body = React.createElement('div', null,
            ...
            React.createElement('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } },
              ...
              React.createElement('span', { className: 'dsh-music-track-size' }, shiftCount + ' 个班次'),
            ),
            React.createElement('div', { className: 'dsh-music-track-size', style: { marginTop: 8 } },
              'ⓘ 定时由插件在后台自维护，保存即生效（无需同步）。每次到点/手动执行都会新建一个执行会话收集并绑定结果；删除某期新闻会连同删除它对应的执行会话。'),
          );
        }
      } else {
        body = listBody;
