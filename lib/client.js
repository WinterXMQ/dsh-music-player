...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PREF_KEYS = new Set([
      ...
      PREF_LYRIC_FX, PREF_LYRIC_PANEL_GHOST,
    ]);
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 几何）、去掉投影，歌词像直接显示在背景页面上。文字阴影用宿主背景令牌
      // --dsw-alias-bg-overlay 调色：深色主题下是暗影（悬浮在暗底上仍可读），浅色
      // 主题下阴影随令牌变浅、视觉上几乎无痕——写死黑色会在浅色主题留灰晕。
      ...
      '.dsh-music-lyric-panel.ghost:hover, .dsh-music-lyric-panel.ghost:focus-within { border-color: var(--dsw-alias-border-l2, rgba(128,128,128,0.35)); }\n' +
      '.dsh-music-lyric-panel.ghost .dsh-music-lyric-panel-body { text-shadow: 0 1px 3px color-mix(in srgb, var(--dsw-alias-bg-overlay, #1e1f22) 60%, transparent); }\n' +
