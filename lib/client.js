...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const store = {
      ...
      playing: false, position: 0, duration: 0, volume: 0.8, muted: false,
    ...
    function changeVolume(value) {
      ...
      audio.volume = v;
      // dragging the slider implies the user wants sound again
    ...
    function toggleMute() {
      if (store.muted) {
        ...
        // mute: keep store.volume as the restore level, silence the audio
        set({ muted: true });
        audio.volume = 0;
      }
    }
    ...
    function VolumeSlider() {
      ...
      return React.createElement('div',
        { className: 'dsh-music-vol-slider' + (s.muted ? ' muted' : ''), ref: trackRef,
          onPointerDown, onPointerMove, onPointerUp,
          title: (s.muted ? '\u5df2\u9759\u97f3\uff08\u97f3\u91cf\u4fdd\u7559 ' + pct + '%\uff09\uff0c\u70b9\u51fb\u53d6\u6d88' : '') + (s.muted ? '' : '\u97f3\u91cf ' + pct + '%') },
        React.createElement('div', { className: 'dsh-music-vol-track' }),
        React.createElement('div', {
          className: 'dsh-music-vol-fill',
          style: { height: pct + '%' },
        }),
        React.createElement('div', {
          className: 'dsh-music-vol-thumb',
        ...
        React.createElement('div', { className: 'dsh-music-bar' },
          ...
          React.createElement('div', { className: 'dsh-music-bar-vol', ref: volRef },
            ...
            volOpen ? React.createElement('div', { className: 'dsh-music-bar-vol-pop' },
              ...
              React.createElement('button', {
                className: 'dsh-music-mute-btn' + (s.muted ? ' active' : ''),
                title: s.muted ? '\u9759\u97f3\uff0c\u70b9\u51fb\u53d6\u6d88' : '\u9759\u97f3',
                onClick: toggleMute,
              }, React.createElement('svg', {
                viewBox: '0 0 24 24', width: 18, height: 18, fill: 'currentColor', 'aria-hidden': true,
              },
                React.createElement('path', { d: 'M3 9v6h4l5 5V4L7 9H3z' }),
                s.muted
                  ? [
                    React.createElement('path', { key: 'x1', d: 'M14 6l7 7 1.5-1.5-7-7z' }),
                    React.createElement('path', { key: 'x2', d: 'M21 6l-7 7 1.5 1.5 7-7z' }),
                  ]
                  : React.createElement('path', { d: 'M13.5 7c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z' }),
              )),
            ) : null,
          ),
          React.createElement('button', {
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-bar-vol-pop { position: absolute; bottom: calc(100% + 6px); left: 50%; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; justify-content: flex-start; gap: 6px; width: 42px; padding: 8px 0; background: var(--dsw-alias-bg-overlay, #1e1f22); border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.35)); border-radius: 8px; box-shadow: 0 8px 20px rgba(0,0,0,0.3); z-index: 60; }\n' +
      ...
      '.dsh-music-vol-slider.muted .dsh-music-vol-fill { opacity: 0.35; }\n' +
      '.dsh-music-vol-slider.muted .dsh-music-vol-thumb { opacity: 0.5; }\n' +
      '.dsh-music-mute-btn { display: inline-flex; align-items: center; justify-content: center; width: 30px; height: 30px; border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.3)); border-radius: 8px; background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.05)); color: var(--dsw-alias-label-secondary, #8a8f98); cursor: pointer; }\n' +
      '.dsh-music-mute-btn:hover { background: var(--dsw-alias-bg-layer-2, rgba(255,255,255,0.09)); }\n' +
      '.dsh-music-mute-btn.active { background: var(--dsh-music-accent, #2f9e6e); border-color: var(--dsh-music-accent, #2f9e6e); color: #fff; }\n' +
