...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resetLyric() {
      ...
      subtitleWeights = [];
      subtitleDur = 0;
      if (store.lyricText !== '' || store.lyricScan !== null) set({ lyricText: '', lyricScan: null });
    }
    ...
    // blur=模糊浮入(Apple Music 风)；karaoke=扫色(整行匀速点亮，走 lyricScan 时间轴)。
    // 跑马灯与边缘渐隐为内置行为（恒开，不是配置项）：超宽行自动滚动，两端渐隐。
    function BarLyric({ text, fx, playing, scan }) {
      ...
      const runStyle = mqOn
        ...
        : undefined;
      const karStyle = (fx === 'karaoke' && scan && Number(scan.dur) > 0)
    ...
    const PLAYER_CSS = '\n' +
      ...
      '@media (prefers-reduced-motion: reduce) { .dsh-music-bar-lyric-run.mq, .dsh-music-bar-lyric-fx { animation: none !important; } .dsh-music-bar-lyric-run.mq { text-align: left; } .dsh-music-bar-lyric-fx[data-prev]::after { content: none; } }\n' +
