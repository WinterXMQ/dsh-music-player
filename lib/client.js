  ...
  factory: (require) => {
    ...
    function QQOnlinePanel() {
      ...
      async function loadTopSongs(top) {
        ...
        const d = await json('/dsh-music/qq/top-songs?topId=' + encodeURIComponent(top.id));
        if (d && d.ok) setTopDetail({ ...d.toplist, id: top.id, name: top.name, cover: top.cover || '' }); else setBrowseErr((d && d.error) || '加载榜单失败');
        setTopLoading(false);
      }
      ...
      async function openPlaylist(pl) {
        ...
        if (d && d.ok) setActivePl({ ...d.playlist, source: pl.name || '歌单' }); else setBrowseErr((d && d.error) || '加载失败');
        setPlLoading(false);
      }
      function openQueue() {
      ...
      async function doSearch(kwOverride) {
        ...
        if (songs && songs.ok) setResults(songs.results || []); else setQError((songs && songs.error) || '歌曲搜索失败');
        if (pls && pls.ok) setPlResults(pls.playlists || []);
      ...
      const songRow = (song, queue, sourceLabel) => {
        ...
        return React.createElement('button', {
          key: id, className: 'dsh-music-track' + (active ? ' active' : ''), title: song.title + ' - ' + artists,
      ...
      if (!loggedIn) {
        return React.createElement('div', { className: 'dsh-music-qq dsh-music-qq-login' },
          React.createElement('div', { className: 'dsh-music-qq-login-center' },
            ...
            React.createElement('p', { className: 'dsh-music-qq-login-warn' },
              '免责声明：扫码登录走非官方接口，仅供个人试听。登录及播放存在账号风控、封禁与合规风险，请知悉并自行承担后果；如有顾虑请勿登录。')),
          loginModal);
      }
            ...
            content = React.createElement('div', null,
              ...
              React.createElement('div', { className: 'dsh-music-qq-topdetail-head' },
                ...
                React.createElement('div', null,
                  ...
                  React.createElement('div', { className: 'dsh-music-hint' }, topDetail.updateTime ? '更新于 ' + topDetail.updateTime : ''))),
              rows);
          } else {
      ...
      const head = React.createElement('div', { className: 'dsh-music-qq-head' },
        React.createElement('div', { className: 'dsh-music-qq-toolbar' },
          React.createElement('button', { className: 'dsh-music-settings-btn ghost', onClick: openQueue }, '进入播放列表'),
    ...
    function PlayerPanel() {
      ...
      const listBody = React.createElement('div', null,
        ...
        React.createElement('div', { className: 'dsh-music-qq-pane', style: paneStyle('qq') }, React.createElement(QQOnlinePanel, null)),
      ...
      return React.createElement('div', { className: 'dsh-music-panel', ref: panelRef, style: rootStyle },
        ...
        s.tab === 'qq' ? null : React.createElement(DirectorySetting, null),
    ...
    const PLAYER_CSS = '\n' +
      ...
      // QQ 面板所在 pane 撑满列表区高度（flex:1 + min-height:0 + overflow:hidden），
      // 让 .dsh-music-qq 有确定高度、.dsh-music-qq-body 的 overflow-y:auto 真正生效：
      // 播放列表 UI 只滚动歌曲列表，head（返回按钮/歌单名）固定不滚。
      '.dsh-music-qq-pane { flex: 1; min-height: 0; overflow: hidden; display: flex; flex-direction: column; }\n' +
      '.dsh-music-qq-head { flex: none; z-index: 3; background: var(--dsw-alias-bg-overlay, #1e1f22); padding-bottom: 4px; }\n' +
      ...
      '.dsh-music-qq-login-warn { font-size: 12px; color: var(--dsw-alias-state-warn-primary, #d9a441); text-align: center; line-height: 1.6; margin-top: 4px; }\n' +
