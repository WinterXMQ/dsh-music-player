  ...
  factory: (require) => {
    ...
    function updateLyric() {
      ...
      if (String(store.currentId).startsWith('book:')) {
        ...
        if (musicWordLyric.length > 0) {
          // —— QRC 精确行窗口：每行自带真实的结束时刻（t..end）。定位用二分（行按
          // t 升序，来自 Host 解析）。扫描进度走「音频时钟驱动」：每次 timeupdate 都
          // 用 audio.currentTime 重算 elapsed 下发，渲染层据此直接设渐变位置——
          // 不再依赖 CSS 动画的墙钟计时，起播缓冲/卡顿/行内 seek 造成的任何漂移
          // 都在下一拍被拉回正确位置；间奏期文本不变也持续校准，不会摊平。
          ...
          const scanInfo = wline ? scanMs(
            ...
            Math.max(0, (now2 - Number(wline.t)) * 1000)) : null;
          if (scanInfo) scanInfo.baseT = Number(wline.t);
          // 音频时钟模式：每拍无条件下发（连续校准是本设计的核心）；无行数据时维持
          // 旧的「文本变化才下发」节流。
          if (scanInfo) set({ lyricText: text, lyricScan: scanInfo });
          else if (text !== store.lyricText) set({ lyricText: text, lyricScan: null });
        } else {
    ...
    function BarLyric({ text, fx, playing, scan, src }) {
      ...
      const runStyle = mqOn
        ...
        : undefined;
      // 卡拉OK两种驱动模式：
      // ① 音频时钟（scan.baseT 存在 = QRC 行窗口）：不做 CSS 关键帧动画，直接按
      //    elapsed/dur 设 background-position-x（updateLyric 每次 timeupdate 重算），
      //    CSS 短过渡把 ~4Hz 的 timeupdate 步进抹平。位置锚定音频时钟，天然免疫
      //    起播缓冲/卡顿/行内 seek 漂移；间奏期停在满亮。
      // ② 整行动画（普通 LRC 兜底）：维持原有 --kar-dur/--kar-delay 墙钟动画
      //    （没有逐行精确数据，两者精度等价）。
      const audioClock = fx === 'karaoke' && scan && Number.isFinite(scan.baseT) && Number(scan.dur) > 0;
      let karStyle;
      if (audioClock) {
        const p = Math.min(1, Math.max(0, (Number(scan.elapsed) || 0) / Number(scan.dur)));
        karStyle = { backgroundPositionX: ((1 - p) * 100).toFixed(2) + '%' };
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 音频时钟驱动模式（QRC 行窗口）：停用墙钟关键帧动画，位置由内联
      // background-position-x 每拍直写；0.24s 线性过渡抹平 timeupdate 的 ~4Hz 步进。
      ".dsh-music-bar-lyric-fx[data-fx='karaoke'][data-audioclock] { animation: dsh-kar-in 0.22s ease backwards; transition: background-position-x 0.24s linear; }\n" +
