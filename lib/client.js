  ...
  factory: (require) => {
    ...
    function KGOnlinePanel({ panelRef }) {
      ...
      const plCard = (pl, mine) => {
        ...
        // 「我的歌单」卡片：收藏的歌单标「收藏」角标、展示原作者；自建歌单标「自建」。
        const kindTag = isMine ? React.createElement('span', {
          className: 'dsh-music-online-tag' + (isCollect ? ' collect' : ''),
        ...
        // 收藏的歌单（别人所有）不提供删除；系统默认歌单（默认收藏/我喜欢）也不可删。
        if (isCollect || pl.isDefault) return card;
