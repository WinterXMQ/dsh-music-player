...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PREF_KEYS = new Set([
      ...
      PREF_PLAYBACK, PREF_PLAYBACK_QQ, PREF_BOOKS_PLAYBACK, PREF_QQ_FAV, PREF_QQ_HISTORY, PREF_QQ_UI,
    ...
    function stop() {
      ...
      // PREF_PLAYBACK_QQ——彼此独立，停本地不会丢掉在线队列，反之亦然。
    ...
    function SystemSetting() {
      ...
      // 卡片内行之间用细分隔线，视觉上归为一类。
      const configCard = (title, ...children) => React.createElement('div', { className: 'dsh-music-config-card' },
        React.createElement('div', { className: 'dsh-music-config-card-title' }, title),
        children);
      return React.createElement('div', { className: 'dsh-music-config' },
        ...
        toggleRow('播放条背景显示', '显示播放条边框与背景色（关闭后仅保留歌名/歌词等内容）', s.showBarBg, (v) => set({ showBarBg: v })),
        immerseRow,
      );
    }
    ...
    const PLAYER_CSS = '\n' +
      ...
      // 分组卡片：整体一个外框，卡片内行之间用细分隔线（不各自带边框）。
      ...
      '.dsh-music-config-card-title { padding: 10px 12px 2px; font-size: 12px; font-weight: 600; color: var(--dsw-alias-label-secondary, #8a8f98); }\n' +
