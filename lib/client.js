...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 把一句可能超长的句子按「最多 SUBTITLE_MAX 字」自适应断成多行：优先在逗号/
    // 顿号等自然停顿处换行，绝不切在引号内、也不切断代理对。
    function wrapSubtitleLine(sentence) {
      ...
      let k = 0;
      while (k < n) {
        if (isQuoteOpenC(sentence[k])) {
          const close = sentence.indexOf(SPLIT_QUOTE_PAIRS[sentence[k]], k + 1);
          if (close !== -1) {
      ...
      let start = 0;
      while (start < n) {
        let end = Math.min(n, start + SUBTITLE_MAX);
        if (end < n && isSubLowSurrogate(sentence.charCodeAt(end))) end--;
        let brk = -1;
        for (let j = start; j < end; j++) {
          if (!inQuote[j] && SUBTITLE_CLAUSE_BREAKS.indexOf(sentence[j]) !== -1) brk = j;
        }
        let lineEnd;
        if (brk > start) {
          lineEnd = brk + 1;
          ...
          lineEnd = outside > start ? outside + 1 : Math.max(start + 1, end);
        }
    ...
    function splitSentences(text) {
      ...
      while (i < raw.length) {
        ...
        if (isQuoteOpenC(c)) {
          ...
          if (close !== -1) { i = close + 1; continue } // 整段对话看作一体，跳过
        }
        if ('。！？…\n'.indexOf(c) !== -1) {
      ...
      if (tail !== '') out.push(...wrapSubtitleLine(tail));
      return out;
    }
    // 单行显示权重 = 行内字数 + 标点停顿补偿（句末停顿 > 分句停顿）。TTS 语速大致
    // 恒定（每秒若干字），所以「显示窗口 ∝ 字数」远优于「按行数均分」；标点停顿让
    // 句号/逗号处的自然停顿也计入时长，进一步贴合真实朗读。
