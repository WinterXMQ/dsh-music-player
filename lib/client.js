...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 章节目录弹层专用锚定：与音量/播放顺序弹窗同款「按钮正上方」效果，但章节列表
    // 高度可变（远超过固定 108px 的小弹窗）。anchorAbove 用 top+translateY(-100%)，
    // 弹窗过高时会顶到视口顶被截断、且 top 被 clamp 后底边脱离按钮。这里改用
    // bottom 锚定：底边始终贴住按钮上方 6px（绝不脱开），高度限制为「视口内可用
    // 空间 ∩ 合理上限（60vh / 480px）」，保证弹窗完整可见且紧贴播放条。
    const tocAnchorAbove = (el, maxW = 380) => {
      const vw = window.innerWidth, vh = window.innerHeight;
      const r = (el && typeof el.getBoundingClientRect === 'function') ? el.getBoundingClientRect() : null;
      const margin = 8;
      const cx = (r && r.width > 0) ? r.left + r.width / 2 : vw / 2;
    ...
    function updateLyric() {
      ...
      if (String(store.currentId).startsWith('book:')) {
        ...
        const idx = Math.min(subtitleLines.length - 1, Math.floor(p * subtitleLines.length));
    ...
    function BookTocPanel({ anchorRef }) {
      ...
      // 用 tocAnchorAbove（bottom 锚定 + 高度限制），避免章节多时弹窗过高顶到
      // 视口顶被截断、底边脱离播放条——与音量/播放顺序弹窗保持一致的紧贴效果。
      return portalToBody(React.createElement('div', { className: 'dsh-music-toc', ref, style: tocAnchorAbove(anchorRef ? anchorRef.current : null) },
