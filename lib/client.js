...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function playBook(id, from = 0) { unlockAutoplay(); bookTotal = -1; bookBufferedFrom = -1; bookBaseTime = 0; bookRestorePos = -1; clearPref(PREF_BOOK_PLAYBACK); playBookFrom(id, from, false); }
    ...
    function step(delta) {
      ...
      if (store.mode === 'shuffle' && store.tracks.length > 1) {        // Walk the shuffled queue: next plays the next unplayed track, prev
