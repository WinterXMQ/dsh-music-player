...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function NewsPane({ panelRef }) {
      ...
      // 班次卡片/弹窗：范围摘要（继承默认时带「默认 · 」前缀）。
      const shiftScopeLabel = (sh) => {
        const scope = sh && sh.scope;
        const cats = scope ? (scope.categories || []) : PRESET_CATEGORIES;
        const topics = (scope && scope.topics) ? scope.topics : [];
        const s = (cats.length > 0 ? cats.join('/') : '未选类别') + (topics.length > 0 ? ' + ' + topics.join(',') : '');
        return (scope ? '' : '默认 · ') + s;
      };
      ...
      const openShiftEditor = (index) => {
        ...
        setShiftEditor(existing === undefined || existing === null
          ? { index: null, draft: { id: 's' + Math.random().toString(36).slice(2, 8), time: '08:00', autoplay: true, scope: null } }
          : { index, draft: { ...existing, scope: existing.scope ? JSON.parse(JSON.stringify(existing.scope)) : null } });
      };
        ...
        if (p === null) {
          ...
          const modelRow = models !== null && models.length > 0 ? (() => {
            ...
            return React.createElement('div', { style: { margin: '4px 0 10px' } },
              React.createElement('div', { style: { fontWeight: 600, marginBottom: 4 } }, '新闻会话模型'),
          ...
          const shiftModal = shiftEditor !== null ? (() => {
            ...
            const setDraftScope = (patch) => {
              const base = d.scope || { categories: (p.defaultScope && p.defaultScope.categories) || PRESET_CATEGORIES.slice(), topics: [] };
            ...
            const scopeCats = (d.scope && d.scope.categories) || (p.defaultScope && p.defaultScope.categories) || PRESET_CATEGORIES;
                ...
                React.createElement('div', { className: 'dsh-music-news-field' },
                  ...
                  React.createElement('div', { style: { display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 } },
                    ...
                    topics.length < LIMITS_NEWS.topicsPerShift ? React.createElement('input', {
                      value: topicDraft, placeholder: '+ 主题，回车添加', className: 'dsh-music-news-time-input',
                      style: { width: 130 },
                    ...
                    React.createElement('button', { className: 'dsh-music-settings-btn ghost', style: { marginLeft: 8 }, onClick: () => setDraft({ scope: null }) }, '重置为默认'),
                  ),
                ),
                React.createElement('div', { className: 'dsh-music-picker-foot' },
                  React.createElement('button', { className: 'dsh-music-settings-btn', onClick: commit, disabled: isNew && shiftCount >= LIMITS_NEWS.shifts }, isNew ? '添加' : '保存'),
