...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
          ...
          React.createElement('div', { className: 'dsh-music-bar-controls' + (active ? ' on' : '') },
            hasTrack
              ? (showHint
                  ...
                  : (isBook ? null : (!barDimmed ? React.createElement('span', { className: 'dsh-music-bar-time' }, fmtTime(s.position) + ' / ' + fmtTime(s.duration)) : null)))
              : null,
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-bar { box-sizing: border-box; display: flex; align-items: center; gap: 8px; width: 100%; max-width: var(--dsh-composer-card-max-width, 780px); margin: 0 auto; padding: 4px 10px; font-size: 12px; color: var(--dsw-alias-label-secondary, #8a8f98); background: var(--dsw-alias-bg-layer-1, rgba(0,0,0,0.04)); border: 1px solid var(--dsw-alias-border-l1, rgba(128,128,128,0.2)); border-radius: 8px; cursor: default; user-select: none; transition: opacity 0.3s ease; }\n' +
      ...
      // 避免在按钮收起过程中抢占/重叠；min-width:0 + 居中 + 省略号保证任意宽度下都
      // 在频谱与时长之间的剩余空间内水平居中。
