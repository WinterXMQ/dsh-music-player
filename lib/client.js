...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BarLyric({ text, fx, playing, scan, src, onDoubleClick }) {
    ...
    function NowPlayingBar() {
      ...
      // 双击名称区域打开面板：歌名/歌手/章节等附属信息都触发；唯独音质徽章
      // （.dsh-music-bar-src）不触发——它是纯信息展示，双击不应打开面板。
        ...
        React.createElement('div',
          ...
          s.lyricText && barDimmed && hasTrack && s.showLyric
            ? React.createElement(BarLyric, {
                ...
                src: s.lyricSource,
                onDoubleClick: () => set({ lyricPanelOpen: !store.lyricPanelOpen }),
              })
            : null,
      ...
      return React.createElement('div', { className: 'dsh-music-panel', ref: panelRef, style: rootStyle },
        ...
        React.createElement('div', { className: 'dsh-music-panel-body' },
          ...
          React.createElement('div', { className: 'dsh-music-tabs' },
            tabBtn('music', '本地音乐'), tabBtn('qq', 'QQ音乐'), tabBtn('kg', '酷狗音乐'), tabBtn('book', 'AI讲书'), tabBtn('config', '系统配置'), tabBtn('about', '关于')),
          React.createElement('div', { className: 'dsh-music-panel-content' },
            (s.tab === 'qq' || s.tab === 'kg' || s.tab === 'config' || s.tab === 'about') ? null : React.createElement(DirectorySetting, { panelRef }),
            ...
            !isBook && s.tab !== 'config' && s.tab !== 'about' && s.loading ? React.createElement('div', { className: 'dsh-music-loading' }, '扫描中…') : null,
    ...
    function LyricPanel() {
      ...
      return React.createElement('div', { className: 'dsh-music-lyric-panel', ref: panelRef, style: rootStyle },
        React.createElement('div', {
          className: 'dsh-music-panel-head dsh-music-panel-drag',
          onPointerDown: onHeadDown, onPointerMove: onHeadMove, onPointerUp: onHeadUp,
        },
      ...
      return React.createElement('div', { className: 'dsh-music-about-top' },
        React.createElement('div', { className: 'dsh-music-about-head' },
          ...
          React.createElement('div', { className: 'dsh-music-about-title' },
            'DSH音乐播放器',
            React.createElement('span', { className: 'dsh-music-about-ver' }, 'v' + (s.version || '—')))),
    ...
    const PLAYER_CSS = '\n' +
      ...
      '.dsh-music-lyric-panel { position: fixed; left: 50%; top: 50%; transform: translate(-50%, -50%); width: 420px; height: 40vh; max-height: 80vh; display: flex; flex-direction: column; gap: 8px; padding: 12px; background: var(--dsw-alias-bg-overlay, #1e1f22); border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,0.35)); border-radius: 12px; box-shadow: 0 12px 32px rgba(0,0,0,0.35); color: var(--dsw-alias-label-primary, #e6e6e6); font-size: 13px; z-index: 1000; pointer-events: auto; overflow: hidden; }\n' +
      // 歌词面板正文：可滚动歌词列表，逐行显示。行（.dsh-music-lyric-line）是块级，
      // 仅承载布局与行距；文字放内联 span（.dsh-music-lyric-line-text）承载字号与
      // 高亮背景——背景只包裹文字长度（短歌名背景短、长歌词换行时由 box-decoration-
      // break: clone 分段带圆角），而非撑满整行。无背景/颜色过渡，换行高亮即时切换。
      // 面板本身保持 13px（标题/按钮等控件字号不变）。
      '.dsh-music-lyric-panel-body { flex: 1; min-height: 0; overflow-y: auto; display: flex; flex-direction: column; gap: 2px; padding: 4px 8px; scrollbar-gutter: stable; }\n' +
      '.dsh-music-lyric-line { padding: 3px 0; }\n' +
      '.dsh-music-lyric-line-text { font-size: 16px; line-height: 1.6; color: var(--dsw-alias-label-secondary, #8a8f98); white-space: pre-wrap; word-break: break-word; }\n' +
      '.dsh-music-lyric-line.active .dsh-music-lyric-line-text { color: var(--dsh-music-accent, #2f9e6e); background: color-mix(in srgb, var(--dsh-music-accent, #2f9e6e) 16%, transparent); font-weight: 600; padding: 2px 8px; -webkit-box-decoration-break: clone; box-decoration-break: clone; }\n' +
      ...
      // 音符图标：固定品牌绿底 + 固定白音符（同「系统配置」开关的固定色方案，而非
      // 随主题反转的 --dsh-music-accent）——深色主题下 brand-primary 会变浅，白字
      // 白底将不可见；固定绿白保证深/浅主题下 Logo 都清晰可辨。
      '.dsh-music-about-logo { flex: none; width: 42px; height: 42px; border-radius: 10px; display: flex; align-items: center; justify-content: center; background: #2f9e6e; color: #fff; font-size: 20px; }\n' +
