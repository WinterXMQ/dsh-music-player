...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function karaokeFrame() {
      ...
      if (karScanNode !== null && musicWordLyric.length > 0) {
        ...
        const wl = wi2 >= 0 ? musicWordLyric[wi2] : null;
        if (wl && Number(wl.end) > Number(wl.t)) {
          const durMs2 = Math.max(300, (Number(wl.end) - Number(wl.t)) * 1000);
