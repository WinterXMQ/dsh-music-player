...
> 目标：在 dsh-music-player 的音乐播放之上新增「自建歌单」。用户可把本地音乐文件组织成多个歌单，
...
| 持久化 | Host 端 `~/.dsh/music-player-playlists.json`（node:fs 直写，沿用 `music-player-state.json` 模式） |
| 加歌交互 | 歌单详情「添加歌曲」→ **文件系统级多选框**（浏览/上级/跨盘符/多选），也可从任意目录选非曲库文件 |
| 默认歌单 | 系统默认「我最喜欢」`pl-fav`：固定第二位、不可重命名、不可删除（可清空），作为收藏总集 |
| 附加能力 | `music_play` 支持 `playlist` 参数；歌单内排序（上移/下移按钮，**明确不做拖拽排序**）；一键清空（含系统歌单） |
| 收藏 | 播放条爱心按钮：当前曲收藏/取消收藏于「我最喜欢」 |

## 2. UI 结构（最终）

**播放面板**：顶部仍为 `音乐 | 小说` 两个大 tab。
...
| 路由 | 方法 | 说明 |
|---|---|---|
| `GET /dsh-music/files?path=` | GET | 文件多选器：列目录 + 音频文件（多选勾选），复用目录浏览体验 |
| `GET /dsh-music/file?path=` | GET/HEAD | 通用流式路由（Range/seek）；**仅放行已登记路径**（歌单成员 ∪ 曲库内路径）防任意访问 |
| `POST /dsh-music/playlist` | POST | `{name}` 新建（空名拒绝；重名允许） |
| `POST /dsh-music/playlist/rename` | POST | `{id,name}`；`fixed` 歌单拒绝 |
| `POST /dsh-music/playlist/delete` | POST | `{id}`；`fixed` 歌单拒绝 |
| `POST /dsh-music/playlist/add` | POST | `{id,paths:[]}` 按路径去重、追加到末尾，跳过无效/非音频 |
| `POST /dsh-music/playlist/remove` | POST | `{id,paths:[]}` 移出歌曲 |
| `POST /dsh-music/playlist/clear` | POST | `{id}` 一键清空（移出全部含已失效；任何歌单都可用，fixed 也可清空） |
| `POST /dsh-music/playlist/reorder` | POST | `{id,paths:[]}` 全量顺序替换（补回未提及的旧成员，防丢歌） |
| `GET /dsh-music/manifest`（扩展） | GET | 追加 `playlists:[{id,name,fixed,count,missing,tracks:[{id,name,url,size,path}]}]`；`pl-fav` 首次启动自动创建 |

### 数据模型

```js
// ~/.dsh/music-player-playlists.json
{ version: 1, playlists: [ { id, name, fixed, trackPaths: [absPath, ...], createdAt, updatedAt } ] }
```

- 成员身份 = **绝对路径**（稳定键），不受曲库重扫的 id 变化影响。
- 曲库歌曲仍走 `/dsh-music/<id>`；**歌单成员统一走 `/dsh-music/file?path=`**（路径稳定、URL 稳定）。
- 成员解析：`name = basename(path)`，`size = statSync`；文件已删/不可读 → 计入 `missing` 并从 `tracks` 剔除（存储保留，文件回来自动恢复）。

### 安全

`/dsh-music/file` 仅放行「已登记」路径（属于任一歌单成员或曲库扫描集），避免开放任意路径访问；
`/dsh-music/files` 沿袭 `/dsh-music/dir` 的本地开发定位。
...
> 另：歌单详情新增「清空」按钮（所有歌单可用，含系统「我最喜欢」），对应 Host `POST /dsh-music/playlist/clear`。
