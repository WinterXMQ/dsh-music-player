...
| 附加能力 | `music_play` 支持 `playlist` 参数；歌单内排序（上移/下移按钮，后续可加拖拽） |
...
- **＋**：新建自建歌单（就地输入名 → 追加标签并切入）。
- **歌单标签**：进入详情；详情内可重命名 / 删除。
...
**播放条**：新增爱心按钮。当前曲已收藏显示实心 ❤，未收藏显示 ♡；点击 = 收藏/取消收藏
（增删于「我最喜欢」），状态随当前曲实时切换。

**范围徽标**：范围 = 歌单时显示 `歌单 · 名字` + 关闭按钮（切回全库）。
...
3. 移除/删除正在播放的歌单内歌曲：切到歌单内下一首；歌单空则停止并提示。
